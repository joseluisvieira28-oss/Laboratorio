...
def rec(close=1790253600000, ltf=5):
    ...
    return {
      ...
      "payload_sha256":s.sha(p),"received_at":"2026-09-24T12:40:06+00:00",
...
class T(unittest.TestCase):
    ...
    def test_bad_poc(self):
        r=rec(); r["payload"]["poc_mid"]=200; r["payload_sha256"]=s.sha(r["payload"])
        self.assertEqual(s.audit([r])["status"],"FAIL_CLOSED")
    def test_bad_sha(self):
        r=rec(); r["payload_sha256"]="0"*64
        self.assertEqual(s.audit([r])["status"],"FAIL_CLOSED")
    def test_conflicting_duplicate(self):
        a=rec(); b=rec(); b["payload"]["close"]=999; b["payload_sha256"]=s.sha(b["payload"])
        self.assertEqual(s.audit([a,b])["status"],"FAIL_CLOSED")
    def test_exact_retry(self):
        a=rec(); x=s.audit([a,dict(a)])
        self.assertEqual(x["status"],"PASS"); self.assertEqual(x["exact_duplicate_retries"],1)
