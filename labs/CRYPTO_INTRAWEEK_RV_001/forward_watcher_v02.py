...
def fetch_day_api(sym, day):
    start=int(day.timestamp()*1000)
    end=int((day+pd.Timedelta(days=1)).timestamp()*1000)-1
    url=f"{API}?symbol={sym}&interval=5m&startTime={start}&endTime={end}&limit=1500"
    req=urllib.request.Request(url,headers={"User-Agent":"CryptoLab-CIRV-Forward/0.2"})
    with urllib.request.urlopen(req,timeout=90) as r:
        raw=r.read()
    arr=json.loads(raw)
    if not isinstance(arr,list) or len(arr)<270:
        raise RuntimeError(f"{sym} public-api day incomplete: {day.date()} rows={len(arr) if isinstance(arr,list) else 'NA'}")
    d=pd.DataFrame(arr)
    if d.shape[1]<7: raise RuntimeError("public-api kline schema")
    d=d.iloc[:,:12].copy()
    d.columns=["open_time","open","high","low","close","volume","close_time","quote_volume","trades","taker_base","taker_quote","ignore"]
    d=d[["open_time","close"]]; d["symbol"]=sym
    return d,hashlib.sha256(raw).hexdigest(),url
...
def fetch_history_through(cutoff):
    # Operational V0.2 source remediation:
    # use immutable Binance Vision monthly archives for completed prior months,
    # and Binance USD-M public klines for completed UTC days in the current month.
    # The target UTC day itself is never requested.
    ...
    days=pd.date_range(first_current,cutoff,freq="D",tz="UTC")
    frames=[]; hashes=[]
    archive_tasks=[(s,f"{BASE}/monthly/klines/{s}/5m/{s}-5m-{m}.zip") for s in ASSETS for m in monthly]
    api_tasks=[(s,dt) for s in ASSETS for dt in days]
    def one_archive(t):
        s,u=t
        raw,data=fetch_zip(u)
        return parse_kline(data,s),hashlib.sha256(raw).hexdigest(),u
    with concurrent.futures.ThreadPoolExecutor(max_workers=12) as ex:
        for d,h,u in ex.map(one_archive,archive_tasks):
            frames.append(d); hashes.append((u,h))
    with concurrent.futures.ThreadPoolExecutor(max_workers=6) as ex:
        for d,h,u in ex.map(lambda t: fetch_day_api(t[0],t[1]),api_tasks):
            frames.append(d); hashes.append((u,h))
...
def main():
    ...
    if mode.startswith("sourcecheck:"):
        cutoff=pd.Timestamp(mode.split(":",1)[1],tz="UTC")
        raw,hashes=fetch_history_through(cutoff)
        d=daily_rv(raw)
        out={"forward_id":"CIRV-BTCETH-FORWARD-001","mode":"SOURCECHECK_V02",
             "cutoff":cutoff.isoformat(),"assets":{},"target_outcome_opened":False,
             "source_transport":"Binance Vision monthly + Binance USD-M public API current-month completed days"}
        for s in ASSETS:
            z=d[d["symbol"]==s]
            out["assets"][s]={"valid_days":int(len(z)),"date_min":z["date"].min().isoformat(),
                              "date_max":z["date"].max().isoformat(),"cutoff_present":bool((z["date"]==cutoff).any())}
        Path("cirv_forward_sourcecheck_v02.json").write_text(json.dumps(out,indent=2,sort_keys=True)+"\\n")
