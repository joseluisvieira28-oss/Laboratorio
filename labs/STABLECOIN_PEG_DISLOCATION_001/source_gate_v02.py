#!/usr/bin/env python3
"""Outcome-blind V0.2 Source/Data Gate for STABLECOIN-PEG-DISLOCATION-001.

V0.2 changes only the source calendar/probe dates after official Binance venue-status
reconciliation. It MUST NOT compute or emit prices, peg deviations, returns, forward
convergence, signal counts, thresholds, extrema, or PnL.
"""
...
INACTIVE_START = dt.date(2022, 9, 27)
INACTIVE_END = dt.date(2023, 3, 10)
...
PROBE_DATES = [
    ...
    dt.date(2023, 3, 15), dt.date(2023, 7, 15),
    dt.date(2024, 1, 15), dt.date(2024, 7, 15),
]
EXPECTED_COLS = 12
MIN_PROBE_ROWS = 1000
OUT = Path("stablecoin_peg_source_gate_v02_artifact")
OUT.mkdir(parents=True, exist_ok=True)
UA = "CryptoLab-SourceGate-V02/1.0 (+research-only)"
...
def fetch_checksum(url: str, expected_filename: str):
    ...
    tokens = text.split()
    sha = tokens[0] if tokens else None
...
def check_day(day: dt.date):
    ...
    if INACTIVE_START <= day <= INACTIVE_END:
        raise RuntimeError("Inactive venue date entered required source calendar")
...
def probe_archive(day: dt.date, official_sha: str):
    if day.year >= 2025 or INACTIVE_START <= day <= INACTIVE_END:
        raise RuntimeError("Probe date violates source-calendar firewall")
    ...
    valid = all([checksum_match, bad_member is None, schema_ok, duplicate_count == 0, timestamps_in_day, ascending, enough_rows])
    return {
        "date": day.isoformat(),
        "zip_sha256": got_sha,
...
def main():
    ...
    if len(days) != EXPECTED_ACTIVE_DAYS or len(set(days)) != EXPECTED_ACTIVE_DAYS:
        raise RuntimeError(f"Active calendar invariant failed: {len(days)}")
    if any(d.year >= 2025 for d in days):
        raise RuntimeError("Protected-period firewall violated")
    if any(INACTIVE_START <= d <= INACTIVE_END for d in days):
        raise RuntimeError("Inactive venue interval leaked into active calendar")
    if any(d not in set(days) for d in PROBE_DATES):
        raise RuntimeError("A probe date is outside frozen active calendar")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=24) as ex:
        ...
        for fut in concurrent.futures.as_completed(futs):
            ...
            try:
                ...
                results.append({
                    "date": d.isoformat(), "zip_status": None, "zip_reachable": False,
                    "zip_probe_method": None, "checksum_status": None, "checksum_valid": False,
                    "checksum_sha256": None, "checksum_error": type(e).__name__,
                })
    ...
    probes = []
    preprobe_ok = zip_ok == EXPECTED_ACTIVE_DAYS and checksum_ok == EXPECTED_ACTIVE_DAYS
    technical_probe_error = None
    if preprobe_ok:
        by_date = {r["date"]: r for r in results}
        try:
            for day in PROBE_DATES:
                probes.append(probe_archive(day, by_date[day.isoformat()]["checksum_sha256"]))
        except Exception as e:
    ...
    manifest = {
        ...
        "symbol": SYMBOL,
        "interval": INTERVAL,
        "active_segments": [[a.isoformat(), b.isoformat()] for a, b in SEGMENTS],
        "structural_inactive_interval": [INACTIVE_START.isoformat(), INACTIVE_END.isoformat()],
    ...
    manifest_path = dump("source_manifest_v02.json", manifest)
    manifest_sha = hashlib.sha256(manifest_path.read_bytes()).hexdigest()
    dump("probe_metadata_v02.json", {
    ...
    receipt = {
        ...
        "inactive_interval_requested_as_required_source": False,
    ...
    dump("source_gate_receipt_v02.json", receipt)
    print(json.dumps({
        "classification": classification,
        "active_days": EXPECTED_ACTIVE_DAYS,
