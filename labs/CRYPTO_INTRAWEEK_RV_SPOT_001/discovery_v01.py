...
def main():
    ...
    Path("cirvs_source_receipt_v01.json").write_text(json.dumps(src,indent=2,sort_keys=True)+"\n")
    print(json.dumps({"source":src},sort_keys=True))
    if src["classification"]!="CIRV_SOURCE_FULL":
        out={"lab_id":"CRYPTO-INTRAWEEK-RV-SPOT-001","verdict":"BLOCKED_SOURCE","holdout_2025_opened":False,"year_2026_opened":False}
        Path("cirvs_final_receipt_v01.json").write_text(json.dumps(out,indent=2,sort_keys=True)+"\n"); return 2

    disc,dframes=eval_discovery(daily)
    Path("cirvs_discovery_receipt_v01.json").write_text(json.dumps(disc,indent=2,sort_keys=True)+"\n")
    for s,z in dframes.items(): z.to_csv(f"cirvs_{s.lower()}_discovery_forecasts_v01.csv",index=False)
    print(json.dumps({"discovery":disc},sort_keys=True))
    if disc["verdict"]!="DISCOVERY_PASS":
        out={"lab_id":"CRYPTO-INTRAWEEK-RV-SPOT-001","mve_id":"CIRV-HAR-DOW-BTCETH-SPOT-001",
    ...
    Path("cirvs_holdout_source_receipt_v01.json").write_text(json.dumps(hsrc,indent=2,sort_keys=True)+"\n")
    if hsrc["classification"]!="CIRV_SOURCE_FULL":
        out={"lab_id":"CRYPTO-INTRAWEEK-RV-SPOT-001","verdict":"DISCOVERY_PASS_HOLDOUT_SOURCE_BLOCKED",
    ...
    for s,z in hframes2.items(): z.to_csv(f"cirvs_{s.lower()}_holdout2025_forecasts_v01.csv",index=False)
    out={"lab_id":"CRYPTO-INTRAWEEK-RV-SPOT-001","mve_id":"CIRV-HAR-DOW-BTCETH-SPOT-001",
