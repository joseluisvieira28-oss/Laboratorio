...
DO NOT EXECUTE unless the immutable original source artifact plus the separate
pre-outcome QA remediation artifact jointly establish SOURCE_DATASET_PASS.
...
import csv, hashlib, io, json, math, random, re, time, urllib.request, urllib.error, zipfile
...
QA_REMEDIATION_RUN_ID=34897999268
BOOT_REPS=20000
BOOT_BLOCK=7
SEED=20260914
COST_BASE_BPS=10.0
...
def load_source():
    ...
    qa_receipts=list(QA_ROOT.rglob('SOURCE_QA_REMEDIATION_RECEIPT.json'))
    if len(receipts)!=1 or len(csvs)!=1 or len(qa_receipts)!=1:
        raise RuntimeError(f'evidence chain incomplete source_receipts={len(receipts)} csvs={len(csvs)} qa_receipts={len(qa_receipts)}')
    src=json.loads(receipts[0].read_text())
    qa=json.loads(qa_receipts[0].read_text())
    src_receipt_sha=file_sha(receipts[0]); csv_sha=file_sha(csvs[0])

    # Preserve the original failed QA receipt. Promotion comes only from the
    # separate, pre-outcome immutable remediation receipt.
    if src.get('classification')!='DATA_FAILURE':
        raise RuntimeError(f'original source receipt unexpectedly changed: {src.get("classification")}')
    structural={
      ...
      'no_source_btc':src.get('btc_market_data_accessed') is False,
      'no_source_2025':src.get('access_2025') is False,
      'no_source_2026':src.get('access_2026') is False,
      'source_csv_hash_match':src.get('csv_sha256')==csv_sha,
    }
    if not all(structural.values()): raise RuntimeError(f'original source structural gate failed: {structural}')

    qa_gate={
      'classification_pass':qa.get('classification')=='SOURCE_DATASET_PASS',
      'source_run_match':qa.get('original_source_run_id')==SOURCE_RUN_ID,
      'source_csv_chain_match':qa.get('original_source_csv_sha256')==csv_sha==src.get('csv_sha256'),
      'source_receipt_chain_match':qa.get('original_source_receipt_sha256')==src_receipt_sha,
      'qa_exact_match':qa.get('qa_exact_match_pass') is True,
      'qa_error_zero':qa.get('qa_error_count')==0,
      'qa_mismatch_zero':qa.get('qa_mismatch_count')==0,
      'qa_243_complete':qa.get('qa_calls_logical_completed')==243 and qa.get('qa_calls_logical_expected')==243,
      'qa_27_batches':qa.get('qa_batches_completed')==27 and qa.get('qa_batches_expected')==27,
      'no_qa_btc':qa.get('btc_market_data_accessed') is False,
      'no_qa_2025':qa.get('access_2025') is False,
      'no_qa_2026':qa.get('access_2026') is False,
    }
    if not all(qa_gate.values()): raise RuntimeError(f'QA remediation chain failed: {qa_gate}')
    ...
    return src,qa,receipts[0],qa_receipts[0],csvs[0],structural,qa_gate,rows
...
def acquire_prices(signal_rows):
    ...
    prices={}; manifests=[]
    for y,m in month_iter(date(2022,11,1),date(2024,12,31)):
...
def moving_block_null_p(x,y,beta_obs,block=7,reps=20000,seed=20260914):
    ...
    xc=[v-xb for v in x]; resid=[v-yb for v in y]
    rng=random.Random(seed); ge=0; blocks=math.ceil(n/block)
    for _ in range(reps):
        num=0.0; pos=0
        for __ in range(blocks):
            ...
            for j in range(block):
                if pos>=n: break
                num += xc[pos]*resid[(s+j)%n]; pos+=1
        if num/den>=beta_obs: ge+=1
...
def main():
    ...
    src,qa,src_receipt_path,qa_receipt_path,source_csv_path,structural,qa_gate,signals=load_source()
    prices,manifests=acquire_prices(signals)
    obs=[]
    for r in signals:
        od=r['signal_date']+timedelta(days=1)
        ets=target_ms(od,0,15); xts=target_ms(od,1,15)
        ep=prices[ets]; xp=prices[xts]; ret=xp/ep-1.0
        flow=r['net_flow_usdt']; s=1 if flow>0 else (-1 if flow<0 else 0)
        gross_bps=s*ret*10000 if s else 0.0
        obs.append({'signal_date':r['signal_date'].isoformat(),'outcome_date':od.isoformat(),'net_flow_usdt':flow,'net_flow_billion':flow/1e9,'entry_ts_ms':ets,'entry_price':ep,'exit_ts_ms':xts,'exit_price':xp,'btc_return':ret,'direction':s,'strategy_gross_bps':gross_bps,'net10_bps':gross_bps-COST_BASE_BPS if s else 0.0,'net14_bps':gross_bps-COST_STRESS_BPS if s else 0.0})
    ...
    alpha,beta,_,_,_=ols_beta(x,y); p,boot_ge=moving_block_null_p(x,y,beta,BOOT_BLOCK,BOOT_REPS,SEED)
    ...
    cal={}
    for o in active: cal.setdefault(o['signal_date'][:4],[]).append(o['net14_bps'])
    cal_stats={yr:{'n':len(v),'mean_net14_bps':sum(v)/len(v)} for yr,v in sorted(cal.items())}
    nonneg=sum(v['mean_net14_bps']>=0 for v in cal_stats.values())
    full_year_deep_negative=any(cal_stats.get(yr,{}).get('mean_net14_bps',0)<-10 for yr in ('2023','2024'))
    mean10=sum(n10)/len(n10); mean14=sum(n14)/len(n14); pf14=profit_factor(n14); pf_receipt=pf14 if math.isfinite(pf14) else 'INF'
    primary_pass=(beta>0 and p<0.05); economic_pass=(mean10>0 and mean14>0 and pf14>1.0); stability_pass=(nonneg>=2 and not full_year_deep_negative)
    promotion=primary_pass and economic_pass and stability_pass
    classification='DISCOVERY_SURVIVOR' if promotion else ('NO_STATISTICAL_EDGE' if not primary_pass else ('NEGATIVE_EXPECTANCY' if not economic_pass else 'STABILITY_FAILURE'))
    ...
    manifest_path=OUT/'BINANCE_ARCHIVE_MANIFEST.json'; manifest_path.write_text(json.dumps(manifests,indent=2,sort_keys=True)+'\n')
    protocol=Path('labs/STABLECOIN_EXCHANGE_FLOW_001/FINAL_PRE_DISCOVERY_PROTOCOL_V0.1.md')
    amendment1=Path('labs/STABLECOIN_EXCHANGE_FLOW_001/PRE_DISCOVERY_TECHNICAL_AMENDMENT_001_BOUNDARY_SEARCH.md')
    amendment2=Path('labs/STABLECOIN_EXCHANGE_FLOW_001/PRE_DISCOVERY_TECHNICAL_AMENDMENT_002_QA_REMEDIATION_CHAIN.md')
    receipt={
      'lab_id':LAB,'mve_id':MVE,'classification':classification,'economic_classification':'POSITIVE_NET_EXPECTANCY' if economic_pass else 'NEGATIVE_EXPECTANCY','promotion_candidate':promotion,
      'n':len(obs),'active_strategy_n':len(active),
      'source_evidence_chain':{'original_source_run_id':SOURCE_RUN_ID,'qa_remediation_run_id':QA_REMEDIATION_RUN_ID,'original_source_classification':src.get('classification'),'qa_remediation_classification':qa.get('classification'),'structural_gates':structural,'qa_gates':qa_gate},
      'primary':{'alpha_return':alpha,'beta_return_per_1bn_usdt':beta,'beta_bps_per_1bn_usdt':beta*10000,'bootstrap_method':'restricted-null circular moving-block residual bootstrap','bootstrap_block_days':BOOT_BLOCK,'bootstrap_reps':BOOT_REPS,'seed':SEED,'one_sided_p_beta_le_0':p,'bootstrap_beta_ge_observed_count':boot_ge,'pass':primary_pass},
      'economic':{'mean_gross_bps':sum(gross)/len(gross),'mean_net10_bps':mean10,'mean_net14_bps':mean14,'profit_factor_net14':pf_receipt,'pass':economic_pass},
      ...
      'source_receipt_sha256':file_sha(src_receipt_path),'qa_remediation_receipt_sha256':file_sha(qa_receipt_path),'source_csv_sha256':file_sha(source_csv_path),'protocol_sha256':file_sha(protocol),'technical_amendment_001_sha256':file_sha(amendment1),'technical_amendment_002_sha256':file_sha(amendment2),'observations_sha256':file_sha(obs_path),'binance_manifest_sha256':file_sha(manifest_path),
