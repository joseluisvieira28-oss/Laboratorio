#!/usr/bin/env python3
"""Cross-check 2022-11-11 frozen Binance-USDT basket flows: MEV Blocker vs Blockchair dump.

Source-only validation. No BTC prices, returns, PnL, 2025 or 2026.
"""
from __future__ import annotations
import csv, gzip, hashlib, io, json, urllib.request, urllib.error
from pathlib import Path

OUT=Path('artifacts/stablecoin_exchange_flow_source_crosscheck_20221111_v01')
...
TARGET_END_TS=1668211200  # 2022-11-12 00:00:00 UTC, exclusive

def rpc(method,params,rid,timeout=60):
    body=json.dumps({'jsonrpc':'2.0','method':method,'params':params,'id':rid},separators=(',',':')).encode()
    req=urllib.request.Request(RPC,data=body,headers={'Content-Type':'application/json','User-Agent':'CryptoLab-SEF-crosscheck/0.1'},method='POST')
    try:
...
def get_logs_adaptive(a,b,direction,ridbox,depth=0):
    ridbox[0]+=1; p,h,n=rpc('eth_getLogs',[log_filter(a,b,direction)],ridbox[0],timeout=90)
    ...
    if a>=b or depth>=18:
      raise RuntimeError(f'RPC log query failed irreducibly {direction} {a}-{b}: {p}')
    mid=(a+b)//2
    left,ml=get_logs_adaptive(a,mid,direction,ridbox,depth+1)
    right,mr=get_logs_adaptive(mid+1,b,direction,ridbox,depth+1)
...
def parse_rpc(logs,direction):
    ...
    for x in logs:
      ...
      sender=topic_addr(topics[1]); recipient=topic_addr(topics[2])
      val=int(x.get('data','0x0'),16)
      if direction=='inbound' and recipient in BASKET and sender not in BASKET:
        count+=1; total+=val
      elif direction=='outbound' and sender in BASKET and recipient not in BASKET:
        count+=1; total+=val
...
def main():
    ...
    end_block=find_last_block_before_target()
    rid=[1000]
    inlogs,mi=get_logs_adaptive(START_BLOCK,end_block,'inbound',rid)
    outlogs,mo=get_logs_adaptive(START_BLOCK,end_block,'outbound',rid)
    rpc_tot={'inbound':parse_rpc(inlogs,'inbound'),'outbound':parse_rpc(outlogs,'outbound')}
    req=urllib.request.Request(DUMP,headers={'User-Agent':'CryptoLab-SEF-crosscheck/0.1'})
    with urllib.request.urlopen(req,timeout=240) as r: raw=r.read()
    dump_sha=hashlib.sha256(raw).hexdigest(); dump_tot=parse_dump(raw)
    exact=(rpc_tot['inbound']==dump_tot['inbound'] and rpc_tot['outbound']==dump_tot['outbound'])
    receipt={'lab_id':'STABLECOIN-EXCHANGE-FLOW-001','mve_id':'SEF-BINANCE-PUBLIC-USDT-ETH-1D-001','classification':'SOURCE_CROSSCHECK_EXACT_PASS' if exact else 'SOURCE_CROSSCHECK_MISMATCH','date':'2022-11-11','start_block':START_BLOCK,'end_block':end_block,'rpc':RPC,'rpc_totals':rpc_tot,'blockchair_dump':DUMP,'dump_sha256':dump_sha,'dump_bytes':len(raw),'dump_totals':dump_tot,'rpc_chunk_manifest':mi+mo,'exact_external_flow_match':exact,'access_2025':False,'access_2026':False,'btc_market_data_accessed':False,'returns_computed':False,'pnl_computed':False}
    (OUT/'SOURCE_CROSSCHECK_20221111_RECEIPT.json').write_text(json.dumps(receipt,indent=2,sort_keys=True)+'\n')
