...
def main():
    cov=json.loads(Path("pcu_eth_coverage_receipt_v01.json").read_text())
    if cov["classification"]=="PCU_ETH_COVERAGE_BLOCKED": raise SystemExit("blocked coverage")
    m=pd.read_csv("pcu_eth_cache/metrics.csv"); f=pd.read_csv("pcu_eth_cache/funding.csv"); k=pd.read_csv("pcu_eth_cache/klines.csv")
    ...
    for r in ev.itertuples(index=False):
        ...
        trades.append({"ts":t.isoformat(),"side":int(r.side),"funding":float(r.funding),"oi_growth_24h":float(r.oi_growth_24h),
                       "entry":entry,"exit":exitp,"gross_return":gross,"net10":gross-.001,"net20":gross-.002})
        active_until=exit_t
    ...
    if n:
        ...
        st={"N":n,"long_N":int((d["side"]==1).sum()),"short_N":int((d["side"]==-1).sum()),
    ...
    gates={"N_gte_60":n>=60,"mean_net10_gt_0":n>0 and st["mean_net10"]>0,
    ...
    out={"lab_id":"PERP-CROWDING-UNWIND-ETH-001","mve_id":"PCU-ETH-24H-TRAILING-CROWDING-001",
         ...
         "btc_result_rewritten":False}
    Path("pcu_eth_discovery_receipt_v01.json").write_text(json.dumps(out,indent=2,sort_keys=True)+"\n")
    d.to_csv("pcu_eth_discovery_trades_v01.csv",index=False)
