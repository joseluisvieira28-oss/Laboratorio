...
AUTH=json.loads((ROOT/"CALENDAR_SPREAD_SOURCE_AUTHORITY_V0.1.json").read_text())
OUT=Path("artifacts/btc_dvol_calendar_spread_source_v01");OUT.mkdir(parents=True,exist_ok=True)
BASE="https://history.deribit.com/api/v2"
S=requests.Session();S.headers.update({"User-Agent":"SRC-Crypto-Lab-DVOL-CalendarSource/0.1","Accept":"application/json"})
...
def acquire_overlap(name,a,b):
    step=86400000;cur=a;rows=[];rcs=[]
    while cur<=b:
        e=min(cur+step-1,b);x,rr=slice_trades(name,cur,e);rows.extend(x);rcs.extend(rr);cur=e+1;time.sleep(0.005)
    ded={}
    for i,x in enumerate(rows):
        ts=x.get("timestamp")
        if not isinstance(ts,(int,float)) or int(ts)<a or int(ts)>b or int(ts)>=PROTECTED_MS:raise RuntimeError(f"Out-of-range row {name}")
...
result={"lab_id":AUTH["lab_id"],"source_gate_id":AUTH["source_gate_id"],"classification":None,"access_2025":False,"access_2026":False,
"spread_opened":False,"basis_opened":False,"returns_opened":False,"pnl_opened":False,"live_trading":False,"exchange_mutation":False,"merge_to_main":False}
try:
    contracts,rejected,meta_rc=discover()
    pairs=[];source_rc=[];core_p=0;core_t=0
    qdef=AUTH["qualifying_pair"]
    for i,(near,far) in enumerate(zip(contracts,contracts[1:]),1):
        start=max(near["creation_timestamp"],far["creation_timestamp"],LOWER_MS)
        end=min(near["expiration_timestamp"],far["expiration_timestamp"],PROTECTED_MS)-1
        if end<start:
            pairs.append({"near":near["instrument_name"],"far":far["instrument_name"],"has_overlap":False,"qualifying":False});continue
        n,nrc=acquire_overlap(near["instrument_name"],start,end);f,frc=acquire_overlap(far["instrument_name"],start,end);source_rc.extend(nrc+frc)
        common=sorted(set(n["days"])&set(f["days"]));core_p+=n["present"]+f["present"];core_t+=n["total"]+f["total"]
        qual=(n["count"]>=qdef["minimum_ordinary_public_trades_each_leg"] and f["count"]>=qdef["minimum_ordinary_public_trades_each_leg"] and
              len(n["days"])>=qdef["minimum_unique_active_utc_days_each_leg"] and len(f["days"])>=qdef["minimum_unique_active_utc_days_each_leg"] and
              len(common)>=qdef["minimum_common_active_utc_days"])
        ...
        print(f"CAL_SOURCE_PROGRESS {i}/{len(contracts)-1} {near['instrument_name']}->{far['instrument_name']}",flush=True)
    ...
    g=AUTH["source_gates"];integrity=len(contracts)/(len(contracts)+len(rejected)) if (contracts or rejected) else 0.0
    coverage=core_p/core_t if core_t else 0.0
    checks={"confirmed_contracts_ge_min":len(contracts)>=g["minimum_confirmed_contracts"],"adjacent_overlap_pairs_ge_min":len(overlaps)>=g["minimum_adjacent_overlap_pairs"],
      "qualifying_pairs_ge_min":len(qual)>=g["minimum_qualifying_adjacent_pairs"],"aggregate_common_pair_days_ge_min":common_total>=g["minimum_aggregate_common_contract_pair_days"],
    ...
    result.update({"classification":cls,"confirmed_contracts":len(contracts),"metadata_rejections":len(rejected),"adjacent_pairs":len(pairs),
      "overlap_pairs":len(overlaps),"qualifying_pairs":len(qual),"aggregate_common_pair_days":common_total,"qualifying_near_expiration_quarters":quarters,
      "core_field_coverage":coverage,"metadata_integrity_rate":integrity,"gate_checks":checks,"pair_summaries":pairs,
...
p=OUT/"calendar_spread_source_result.json";p.write_text(json.dumps(result,indent=2,sort_keys=True)+"\n")
(OUT/"manifest.json").write_text(json.dumps({"authority_sha256":sha((ROOT/"CALENDAR_SPREAD_SOURCE_AUTHORITY_V0.1.json").read_bytes()),"result_sha256":sha(p.read_bytes())},indent=2,sort_keys=True)+"\n")
