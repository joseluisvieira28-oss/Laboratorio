...
AUTH_PATH = ROOT / 'LIFECYCLE_SOURCE_AUTHORITY_V0.1.json'
SPEC_PATH = ROOT / 'LIFECYCLE_SOURCE_TECHNICAL_SPEC_V0.1.json'
AUTH = json.loads(AUTH_PATH.read_text())
SPEC = json.loads(SPEC_PATH.read_text())
OUT = Path('artifacts/btc_dvol_futures_lifecycle_source_v01')
OUT.mkdir(parents=True, exist_ok=True)
BASE = 'https://history.deribit.com/api/v2'
S = requests.Session()
S.headers.update({'User-Agent':'SRC-Crypto-Lab-DVOL-LifecycleSource/0.1','Accept':'application/json'})
    ...
    for attempt in range(3):
        try:
            ...
            try: obj=r.json()
                ...
                raise RuntimeError(f'non-JSON HTTP {r.status_code} {path} sha256={digest}: {e}')
            ...
            if r.status_code!=200:
                raise RuntimeError(f'HTTP {r.status_code} {path} sha256={digest}')
            return obj, {'endpoint':path,'status':r.status_code,'sha256':digest,'http_bytes':len(raw)}
        except (requests.RequestException, RuntimeError) as e:
            last=e
            if attempt<2 and isinstance(e,requests.RequestException): time.sleep(waits[attempt]); continue
            raise
...
def wednesdays(start_s,end_s):
    ...
    if d.weekday()!=2 or e.weekday()!=2: raise RuntimeError('frozen candidate bounds are not Wednesday')
    while d<=e:
        yield d
        d += timedelta(days=7)
...
def discover_contracts():
    ...
    for d in wednesdays(cfg['candidate_start'],cfg['candidate_end']):
        ...
        time.sleep(0.015)
    conforming=[x for x in accepted if x['accepted']]
    rejected=[x for x in accepted if not x['accepted']]
    return conforming,rejected,misses,receipts

def trade_slice(name,a,b):
    if a>=PROTECTED_MS or b>=PROTECTED_MS: raise RuntimeError(f'protected-period trade request blocked {name} {a}-{b}')
    ...
    if len(trades)==1000:
        ...
        mid=(a+b)//2
        l,lr=trade_slice(name,a,mid); r,rr=trade_slice(name,mid+1,b)
        return l+r,[rc]+lr+rr
    return trades,[rc]

def acquire_life(meta):
    name=meta['instrument_name']; start=meta['creation_timestamp']; end=meta['expiration_timestamp']
    if start<LOWER_MS or end>=PROTECTED_MS or end<start: raise RuntimeError(f'unsafe lifecycle metadata {name}')
    step=24*60*60*1000; t=start; rows=[]; receipts=[]
    while t<=end:
        e=min(t+step-1,end)
        x,rr=trade_slice(name,t,e); rows.extend(x); receipts.extend(rr); t=e+1
        time.sleep(0.01)
    dedup={}
    for i,x in enumerate(rows):
        key=str(x.get('trade_id',f'MISSING-{i}'))
        ts=x.get('timestamp')
        if not isinstance(ts,(int,float)) or int(ts)<start or int(ts)>end or int(ts)>=PROTECTED_MS:
            raise RuntimeError(f'out-of-bounds trade timestamp in {name}: {ts}')
        dedup[key]=x
    ordinary=[]
    ...
    return {
      ...
      'expiration_timestamp':end,
      'ordinary_public_trade_count':len(ordinary),
      'unique_active_utc_days':len(active_days),
      'first_ordinary_trade_timestamp':first,
      'last_ordinary_trade_timestamp':last,
      'core_field_present_cells':core_present,
      'core_field_total_cells':core_den,
      'aux_field_present_cells':aux_present,
      'aux_field_total_cells':aux_den,
...
def main():
    result={'lab_id':AUTH['lab_id'],'source_gate_id':AUTH['source_gate_id'],'runner':'lifecycle_source_gate_v01.py','access_2025':False,'access_2026':False,'outcomes_opened':False,'basis_opened':False,'convergence_opened':False,'strategy_pnl_opened':False,'live_trading':False,'exchange_mutation':False,'merge_to_main':False}
    try:
        contracts,rejected,misses,meta_receipts=discover_contracts()
        lives=[]
        ...
        for x in lives:
            x['qualifying_source_contract']=(x['ordinary_public_trade_count']>=qdef['minimum_ordinary_public_trades'] and x['unique_active_utc_days']>=qdef['minimum_unique_active_utc_days'])
            x['expiration_calendar_quarter']=quarter(x['expiration_timestamp'])
        qualifying=[x for x in lives if x['qualifying_source_contract']]
        total_trades=sum(x['ordinary_public_trade_count'] for x in lives)
        core_present=sum(x['core_field_present_cells'] for x in lives); core_total=max(1,sum(x['core_field_total_cells'] for x in lives))
        aux_present=sum(x['aux_field_present_cells'] for x in lives); aux_total=max(1,sum(x['aux_field_total_cells'] for x in lives))
        active_contract_days=sum(x['unique_active_utc_days'] for x in lives)
        quarters=sorted({x['expiration_calendar_quarter'] for x in qualifying})
        metadata_integrity=(len(contracts)/(len(contracts)+len(rejected))) if (contracts or rejected) else 0.0
        checks={
          'confirmed_contracts_ge_min':len(contracts)>=g['minimum_confirmed_contracts'],
          'qualifying_contracts_ge_min':len(qualifying)>=g['minimum_qualifying_contracts'],
          'aggregate_active_contract_days_ge_min':active_contract_days>=g['minimum_aggregate_active_contract_days'],
          'qualifying_quarters_ge_min':len(quarters)>=g['minimum_distinct_calendar_quarters_among_qualifying_contracts'],
          'core_field_coverage_ge_min':(core_present/core_total)>=g['core_field_coverage_minimum'],
          'aux_field_coverage_ge_min':(aux_present/aux_total)>=g['aux_field_coverage_minimum'],
        ...
        result.update({
          ...
          'candidate_wednesdays_tested':len(meta_receipts),
          'candidate_miss_count':len(misses),
          'metadata_rejection_count':len(rejected),
          'confirmed_contract_count':len(contracts),
          'qualifying_contract_count':len(qualifying),
          'aggregate_ordinary_public_trade_count':total_trades,
          'aggregate_active_contract_days':active_contract_days,
          'qualifying_expiration_quarters':quarters,
          'core_field_coverage':core_present/core_total,
          'aux_field_coverage':aux_present/aux_total,
          'metadata_integrity_rate':metadata_integrity,
          'gate_checks':checks,
          'contracts':clean,
    ...
    p=OUT/'lifecycle_source_gate_result.json'; p.write_text(json.dumps(result,indent=2,sort_keys=True)+'\n')
    manifest={'authority_sha256':h(AUTH_PATH.read_bytes()),'technical_spec_sha256':h(SPEC_PATH.read_bytes()),'result_sha256':h(p.read_bytes())}
