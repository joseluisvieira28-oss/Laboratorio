...
AUTH=json.loads((ROOT/"TERM_SPREAD_MR_DISCOVERY_AUTHORITY_V0.1.json").read_text())
OUT=Path("artifacts/btc_dvol_termspread_mr_discovery_v01");OUT.mkdir(parents=True,exist_ok=True)
BASE="https://history.deribit.com/api/v2"
S=requests.Session();S.headers.update({"User-Agent":"SRC-Crypto-Lab-DVOL-TermSpreadMR/0.1","Accept":"application/json"})
...
def acquire(name,a,b):
    ...
    for x in ded.values():
        ...
        try:ts=int(x["timestamp"]);p=float(x["price"])
        except Exception:raise RuntimeError(f"Missing field {name}")
        if not(math.isfinite(p) and p>0):raise RuntimeError(f"Invalid value {name}")
        out.append((ts,p))
...
result={"lab_id":AUTH["lab_id"],"discovery_id":AUTH["discovery_id"],"classification":None,
        "access_2025":False,"access_2026":False,"transaction_costs_opened":False,
        "strategy_pnl_opened":False,"strategy_returns_opened":False,
        "live_trading":False,"exchange_mutation":False,"merge_to_main":False}
try:
    contracts,meta_rc=discover();source_rc=[];summ=[];all_scores=[]
    for i,(near,far) in enumerate(zip(contracts,contracts[1:]),1):
        ...
        for d0,d1 in zip(days,days[1:]):
            n0=na[d0][1];f0=fa[d0][1];n1=na[d1][1];f1=fa[d1][1]
            s0=n0-f0;s1=n1-f1
            gap=(date.fromisoformat(d1)-date.fromisoformat(d0)).days
            if gap<=0:continue
            sign=1.0 if s0>0 else (-1.0 if s0<0 else 0.0)
            score=sign*(s0-s1)/gap
            if math.isfinite(score):scores.append(score);all_scores.append(score)
        if scores:
            summ.append({"near":near["instrument_name"],"far":far["instrument_name"],
                         "near_expiration_quarter":qtr(near["expiration_timestamp"]),
                         "score_n":len(scores),"mean_score_per_day":sum(scores)/len(scores),
                         "median_score_per_day":statistics.median(scores),
                         "hit_rate":sum(1 for x in scores if x>0)/len(scores)})
        print(f"TERM_SPREAD_PROGRESS {i}/{len(contracts)-1} {near['instrument_name']}->{far['instrument_name']}",flush=True)
    means=[x["mean_score_per_day"] for x in summ];quarters=defaultdict(list)
    for x in summ:quarters[x["near_expiration_quarter"]].append(x["mean_score_per_day"])
    ...
    sample_checks={"pairs_ge_min":len(summ)>=sg["minimum_pair_clusters_with_scores"],
                   "scores_ge_min":len(all_scores)>=sg["minimum_total_adjacent_scores"],
                   "quarters_ge_min":len(qmeans)>=sg["minimum_distinct_near_expiration_quarters"]}
    metrics={"pair_clusters_with_scores":len(summ),"total_scores":len(all_scores),
             "mean_pair_score_per_day":sum(means)/len(means) if means else None,
             "median_pair_score_per_day":statistics.median(means) if means else None,
             "positive_pair_share":sum(1 for x in means if x>0)/len(means) if means else 0.0,
             "pooled_hit_rate":sum(1 for x in all_scores if x>0)/len(all_scores) if all_scores else 0.0,
             "near_expiration_quarter_means":qmeans,
             "nonnegative_quarters":sum(1 for x in qmeans.values() if x>=0),
    ...
    result.update({"classification":cls,"sample_checks":sample_checks,"statistical_checks":stat_checks,
                   "metrics":metrics,"pair_summaries":summ,
                   "source_request_count":len(meta_rc)+len(source_rc),
...
p=OUT/"TERM_SPREAD_MR_DISCOVERY_RESULT_V0.1.json";p.write_text(json.dumps(result,indent=2,sort_keys=True)+"\n")
(OUT/"manifest.json").write_text(json.dumps({"authority_sha256":sha((ROOT/"TERM_SPREAD_MR_DISCOVERY_AUTHORITY_V0.1.json").read_bytes()),"result_sha256":sha(p.read_bytes())},indent=2,sort_keys=True)+"\n")
