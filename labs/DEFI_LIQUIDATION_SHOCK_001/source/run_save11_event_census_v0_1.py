...
PARENT_RUN=35715830047
PARENT_ARTIFACT=10690805384
PARENT_ARTIFACT_SHA="e72c22a7164ba8787c03201a3d7b2eb1a42a6ffc4384d3dcb8e80695aa9e8c7c"
RPC="https://api.mainnet-beta.solana.com"
TRANCHE_SIZE=250
MAX_RETRIES=12
REPO_DIR=Path("labs/DEFI_LIQUIDATION_SHOCK_001")
QUEUE_PATH=REPO_DIR/"SAVE11_EVENT_CENSUS_QUEUE_V0.1.json"
QUEUE_RECEIPT=REPO_DIR/"SAVE11_EVENT_CENSUS_QUEUE_RECEIPT_V0.1.json"
...
def cmd_queue(parent):
    ...
    receipt["classification"]="SAVE11_EVENT_CENSUS_QUEUE_FROZEN"
...
def cmd_finalize(queue_path,tranches_dir):
    ...
    base=REPO_DIR
    (base/"SAVE11_EVENT_CENSUS_ADJUDICATION_V0.1.jsonl").write_text("".join(json.dumps(x,sort_keys=True)+"\n" for x in adjud),encoding="utf-8")
    (base/"SAVE11_EVENT_CENSUS_EVENTS_V0.1.jsonl").write_text("".join(json.dumps(x,sort_keys=True)+"\n" for x in events),encoding="utf-8")
    (base/"SAVE11_EVENT_CENSUS_FAILED_ATTEMPTS_V0.1.jsonl").write_text("".join(json.dumps(x,sort_keys=True)+"\n" for x in attempts),encoding="utf-8")
    receipt={
      "schema_version":"0.1","lab_id":LAB_ID,"class_id":CLASS_ID,
      "classification":"SAVE11_EVENT_CENSUS_FIRST_CHUNK_PASS",
      "window":{"start":START_ISO,"end":END_ISO,"semantics":"half_open"},
    ...
    write_json(base/"SAVE11_EVENT_CENSUS_CHUNK_RECEIPT_V0.1.json",receipt)
