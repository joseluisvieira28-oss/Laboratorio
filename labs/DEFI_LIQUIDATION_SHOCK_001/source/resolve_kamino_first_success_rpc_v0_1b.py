...
ANCHOR_SIG="47XL1cg6q8WtXxgAhoQGhtRMRm7EnrTtHZUR4QarxxuvpXTxCQANAnrDku7mHukz7JYxSYAquSyWAnH4DKSDiAis"
ANCHOR_SLOT=260478524
ANCHOR_TS=int(datetime.fromisoformat("2024-04-16T15:14:44+00:00").timestamp())
PAGE_LIMIT=1000
MAX_PAGES=5000
MAX_RETRIES=12
OUT=Path("dls_kamino_rpc_boundary_v01b")
...
state={
 "schema_version":"0.1B","lab_id":"DEFI-LIQUIDATION-SHOCK-001",
 "transport":"official_public_solana_rpc","endpoint":RPC,
 "program_id":PROGRAM,"discriminator":PREFIX_HEX,
 "source_boundary":BOUNDARY_ISO,
 "continuation_parent_run_id":35689367726,
 "resume_cursor_extraction_run_id":35702234721,
 "resume_cursor_page_sha256":"26a782c34ce39be0d9542a0bbe3fb16387636b5a070bf7b9ee7ae5aaa41a16d9",
 "anchor":{"signature":ANCHOR_SIG,"slot":ANCHOR_SLOT,"block_time":ANCHOR_TS},
...
for page_no in range(1,MAX_PAGES+1):
    ...
    if not page:
        state.update(phase="BLOCKED",classification="KAMINO_FIRST_SUCCESS_BOUNDARY_RPC_HISTORY_BLOCKED",reason="history_exhausted_before_crossing_lower_boundary")
        write_json("DLS_KAMINO_RPC_FIRST_SUCCESS_RECEIPT_V0.1B.json",state)
...
if not state["crossed_lower_boundary"]:
    state.update(phase="BLOCKED",classification="KAMINO_FIRST_SUCCESS_BOUNDARY_RPC_HISTORY_BLOCKED",reason="max_pages_reached_before_lower_boundary")
    write_json("DLS_KAMINO_RPC_FIRST_SUCCESS_RECEIPT_V0.1.json",state)
    print(json.dumps(state,indent=2)); sys.exit(0)
...
eligible=[r for r in rows if r["blockTime"]>=BOUNDARY_TS]
eligible.append({"signature":ANCHOR_SIG,"slot":ANCHOR_SLOT,"blockTime":ANCHOR_TS,"err":None,"frozen_anchor":True})
...
for pos,row in enumerate(eligible):
    ...
    res=obj.get("result")
    if res is None:
        state.update(phase="BLOCKED",classification="KAMINO_FIRST_SUCCESS_BOUNDARY_RPC_HISTORY_BLOCKED",
    ...
    if int(res.get("slot",-1))!=int(row["slot"]):
        state.update(phase="FAIL_CLOSED",classification="KAMINO_FIRST_SUCCESS_BOUNDARY_SOURCE_ANOMALY_FAIL_CLOSED",
    ...
    if matched:
        ...
        state.update(phase="COMPLETE",classification="KAMINO_FIRST_SUCCESS_BOUNDARY_RPC_PASS",
...
state.update(phase="COMPLETE_NO_MATCH",classification="KAMINO_FIRST_SUCCESS_BOUNDARY_RPC_NO_MATCH_IN_EARLIEST_SLICE",
