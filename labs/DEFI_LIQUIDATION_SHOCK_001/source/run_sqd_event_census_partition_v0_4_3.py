...
def req(url, body=None, retries=20):
    ...
    headers={"Accept":"application/x-ndjson,application/json","User-Agent":"crypto-lab-dls-census/0.4.3"}
...
def stream_day(protocol, day_start, day_end, from_slot, to_slot):
    day_start_ts=int(iso_to_dt(day_start).timestamp())
    day_end_ts=int(iso_to_dt(day_end).timestamp())
    authoritative_ts=int(iso_to_dt(CFG[protocol]["authoritative_start"]).timestamp())
    c=CFG[protocol]
    current=from_slot
    request_count=0
    rows=[]
    ...
    while current<=to_slot:
        ...
        while True:
            ...
            if st==204:
                ...
                break
            if st!=200:
                raise RuntimeError(f"stream_http_{st}:{raw[:500]!r}")
            txt=raw.decode("utf-8","replace")
            lines=[x for x in txt.splitlines() if x.strip()]
            if lines:
                break
            # SQD API spec allows an empty 200 list for a bounded Solana
            # request only when the entire requested slot range is skipped.
            # A normal daily census interval cannot assume that condition,
            # so retry the identical request and fail closed if it persists.
            empty_200_attempt+=1
            if empty_200_attempt>=6:
                raise RuntimeError("empty_200_response_retries_exhausted")
            time.sleep(min(60,2**empty_200_attempt))
        if st==204:
            break
        ...
        for b in batch:
            ...
            # V0.4.2: timestamp endpoint is only a lower seed. Scientific
            # membership is determined locally by exact UTC timestamps.
            if ts is None:
                continue
            try:
                block_ts=int(iso_to_dt(ts).timestamp())
            except Exception:
                continue
            if not (day_start_ts <= block_ts < day_end_ts):
                continue
            if block_ts < authoritative_ts:
                continue
            tx_by={}
    ...
    return {
      "schema_version":"0.4.3","transport_revision":"0.4.2.1","protocol":protocol,"day_start":day_start,"day_end":day_end,
...
manifest={"schema_version":"0.4.3","transport_revision":"0.4.2.1","protocol":args.protocol,"requested_start":args.start,"requested_end":args.end,
...
while cur<end:
    ...
    # Timestamp resolver returns a block at/before the requested timestamp,
    # not necessarily the first slot at/after it. Query a frozen +16 slot
    # envelope and remove spillover by exact local timestamp filtering.
    to_slot=next_slot+16
    ...
    try:
        ...
        rec={"schema_version":"0.4.3","protocol":args.protocol,"day_start":day0,"day_end":day1,
