...
for symbol in SYMBOLS:
    for date in DATES:
        ...
        for family in FAMILIES:
            url=f"{ROOT}/{symbol}/{family}/{year}/{compact}"
            rec={"symbol":symbol,"date":date,"date_key":compact,"family":family}
            try:
                req=urllib.request.Request(url,method="HEAD",headers={"User-Agent":"crypto-lab-dls-source-metadata/0.2"})
...
receipt={
  ...
  "technical_supersession_of":"DRIFT_HISTORICAL_S3_HEAD_CALIBRATION_V0.1",
  "correction":"daily object filename YYYY-MM-DD -> YYYYMMDD",
...
Path("labs/DEFI_LIQUIDATION_SHOCK_001/DRIFT_HISTORICAL_S3_HEAD_CALIBRATION_RECEIPT_V0.2.json").write_text(
