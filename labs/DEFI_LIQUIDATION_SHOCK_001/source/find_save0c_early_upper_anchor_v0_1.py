...
TARGET_ISO="2021-12-15T00:00:00Z"
...
OUT=Path("dls_save0c_early_upper_anchor_v01")
...
for round_no in range(1,MAX_CALIBRATION_ROUNDS+1):
    ...
    if not isinstance(slots,list) or not slots:
        raise RuntimeError("SAVE0C_EARLY_UPPER_ANCHOR_RPC_HISTORY_BLOCKED: no produced slots in calibration bracket")

    lo_t=block_time(slots[0])
    hi_t=block_time(slots[-1])
    if lo_t is None or hi_t is None:
        raise RuntimeError("SAVE0C_EARLY_UPPER_ANCHOR_SOURCE_ANOMALY_FAIL_CLOSED: null block time on produced slot")
    ...
    if int(lo_t)<=TARGET_TS<=int(hi_t):
        ...
        while lo<=hi:
            ...
            if t is None:
                raise RuntimeError(f"SAVE0C_EARLY_UPPER_ANCHOR_SOURCE_ANOMALY_FAIL_CLOSED: null time at {slots[mid]}")
            if int(t)>=TARGET_TS:
                first=mid
                hi=mid-1
            else:
                lo=mid+1
        if first is None:
            raise RuntimeError("SAVE0C_EARLY_UPPER_ANCHOR_SOURCE_ANOMALY_FAIL_CLOSED: target bracket binary search failed")
...
if target_slot is None:
    receipt={
      ...
      "classification":"SAVE0C_EARLY_UPPER_ANCHOR_RPC_HISTORY_BLOCKED",
      "reason":"calibration_rounds_exhausted","target_time":TARGET_ISO,
      "calibration":calibration
    }
    Path("labs/DEFI_LIQUIDATION_SHOCK_001/SAVE0C_EARLY_UPPER_ANCHOR_RECEIPT_V0.1.json").write_text(json.dumps(receipt,indent=2,sort_keys=True)+"\n")
...
if not isinstance(scan_slots,list) or not scan_slots:
    raise RuntimeError("SAVE0C_EARLY_UPPER_ANCHOR_RPC_HISTORY_BLOCKED: no produced scan slots")
...
for slot in scan_slots:
    ...
    if bt is None or int(bt)<TARGET_TS:
        raise RuntimeError(f"SAVE0C_EARLY_UPPER_ANCHOR_SOURCE_ANOMALY_FAIL_CLOSED: chronology {slot} {bt}")
    blocks_scanned+=1
...
classification="SAVE0C_EARLY_UPPER_ANCHOR_PASS" if anchor else "SAVE0C_EARLY_UPPER_ANCHOR_ACTIVITY_NOT_FOUND"
receipt={
  "schema_version":"0.1","lab_id":"DEFI-LIQUIDATION-SHOCK-001",
  "classification":classification,
  "endpoint":RPC,"program_id":PROGRAM,
  "target_time":TARGET_ISO,
  "source_boundary":"2021-12-08T00:00:00Z",
