...
START_ISO="2023-11-18T00:00:00Z"
END_ISO="2023-11-19T00:00:00Z"
...
QUEUE_PATH=REPO_DIR/"KAMINO_V1_EVENT_CENSUS_2023_11_18_QUEUE_V0.1.json"
QUEUE_RECEIPT=REPO_DIR/"KAMINO_V1_EVENT_CENSUS_2023_11_18_QUEUE_RECEIPT_V0.1.json"
...
def cmd_queue(parent):
    ...
    selected=[]; seen=set(); page_count=0; null_bt=0; all_times=[]
    for p in pages:
    ...
    if not all_times:
        raise RuntimeError("NO_TIMESTAMPS_IN_PARENT_ARTIFACT")
    coverage_min=min(all_times); coverage_max=max(all_times)
    if coverage_min > START_TS or coverage_max < END_TS:
        raise RuntimeError(f"PARENT_ARTIFACT_DOES_NOT_BRACKET_WINDOW min={coverage_min} max={coverage_max}")
    ...
    queue={
      ...
      "coverage":{"min_block_time":coverage_min,"max_block_time":coverage_max,"window_bracketed":True},
      "authority_boundary":{"signature":FIRST_SIG,"slot":FIRST_SLOT,"blockTime":1700232504},
      "rows":selected,
    ...
    receipt["classification"]="KAMINO_V1_EVENT_CENSUS_2023_11_18_QUEUE_FROZEN"
...
def cmd_finalize(queue_path,tranches_dir):
    ...
    base=REPO_DIR
    (base/"KAMINO_V1_EVENT_CENSUS_2023_11_18_ADJUDICATION_V0.1.jsonl").write_text("".join(json.dumps(x,sort_keys=True)+"\n" for x in adjud),encoding="utf-8")
    (base/"KAMINO_V1_EVENT_CENSUS_2023_11_18_EVENTS_V0.1.jsonl").write_text("".join(json.dumps(x,sort_keys=True)+"\n" for x in events),encoding="utf-8")
    (base/"KAMINO_V1_EVENT_CENSUS_2023_11_18_FAILED_ATTEMPTS_V0.1.jsonl").write_text("".join(json.dumps(x,sort_keys=True)+"\n" for x in attempts),encoding="utf-8")
    receipt={
      "schema_version":"0.1","lab_id":LAB_ID,"class_id":CLASS_ID,
      "classification":"KAMINO_V1_EVENT_CENSUS_2023_11_18_PASS",
      "window":{"start":START_ISO,"end":END_ISO,"semantics":"half_open"},
      ...
      "first_realized_event":({"signature":first["signature"],"slot":first["slot"],"blockTime":first["blockTime"],"blockTimeIso":first["blockTimeIso"]} if first else None),
      "parent":{"run_id":PARENT_RUN,"artifact_id":PARENT_ARTIFACT,"artifact_sha256":PARENT_ARTIFACT_SHA},
      "firewalls":{"prices":False,"returns":False,"pnl":False,"direction":False,"market_outcomes":False,
                   "event_size_threshold_tuning":False,"live_trading":False,"orders":False,"wallets":False,
                   "exchange_mutation":False,"paid_source":False,"account_creation":False,"merge_main":False}
    }
    write_json(base/"KAMINO_V1_EVENT_CENSUS_2023_11_18_CHUNK_RECEIPT_V0.1.json",receipt)
