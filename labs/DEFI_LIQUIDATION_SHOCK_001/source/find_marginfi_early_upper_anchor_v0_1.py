...
TARGET_ISO="2023-02-14T00:00:00Z"
...
REF_SLOT=239637875
REF_TS=int(datetime.fromisoformat("2024-01-03T12:26:07+00:00").timestamp())

MAX_RETRIES=12
BRACKET_LIMIT=40000
MAX_CALIBRATION_ROUNDS=12
MAX_SCAN_BLOCKS=2500
OUT=Path("dls_marginfi_early_upper_anchor_v01")
...
for round_no in range(1,MAX_CALIBRATION_ROUNDS+1):
    ...
    if not isinstance(slots,list) or not slots:
        raise RuntimeError("MARGINFI_EARLY_UPPER_ANCHOR_RPC_HISTORY_BLOCKED: no produced slots in calibration bracket")

    lo_t=block_time(slots[0])
    hi_t=block_time(slots[-1])
    if lo_t is None or hi_t is None:
        raise RuntimeError("MARGINFI_EARLY_UPPER_ANCHOR_SOURCE_ANOMALY_FAIL_CLOSED: null block time on produced slot")
    ...
    if int(lo_t)<=TARGET_TS<=int(hi_t):
        ...
        while lo<=hi:
            ...
            if t is None:
                raise RuntimeError(f"MARGINFI_EARLY_UPPER_ANCHOR_SOURCE_ANOMALY_FAIL_CLOSED: null time at {slots[mid]}")
            if int(t)>=TARGET_TS:
                first=mid
                hi=mid-1
            else:
                lo=mid+1
        if first is None:
            raise RuntimeError("MARGINFI_EARLY_UPPER_ANCHOR_SOURCE_ANOMALY_FAIL_CLOSED: target bracket binary search failed")
...
if target_slot is None:
    receipt={
      ...
      "classification":"MARGINFI_EARLY_UPPER_ANCHOR_RPC_HISTORY_BLOCKED",
      "reason":"calibration_rounds_exhausted","target_time":TARGET_ISO,
      "calibration":calibration
    }
    Path("labs/DEFI_LIQUIDATION_SHOCK_001/MARGINFI_EARLY_UPPER_ANCHOR_RECEIPT_V0.1.json").write_text(json.dumps(receipt,indent=2,sort_keys=True)+"\n")
...
if not isinstance(scan_slots,list) or not scan_slots:
    raise RuntimeError("MARGINFI_EARLY_UPPER_ANCHOR_RPC_HISTORY_BLOCKED: no produced scan slots")
...
for slot in scan_slots:
    ...
    if bt is None or int(bt)<TARGET_TS:
        raise RuntimeError(f"MARGINFI_EARLY_UPPER_ANCHOR_SOURCE_ANOMALY_FAIL_CLOSED: chronology {slot} {bt}")
    blocks_scanned+=1
...
classification="MARGINFI_EARLY_UPPER_ANCHOR_PASS" if anchor else "MARGINFI_EARLY_UPPER_ANCHOR_ACTIVITY_NOT_FOUND"
receipt={
  "schema_version":"0.1","lab_id":"DEFI-LIQUIDATION-SHOCK-001",
  "classification":classification,
  "endpoint":RPC,"program_id":PROGRAM,
  "target_time":TARGET_ISO,
  "source_boundary":"2023-02-07T15:47:04Z",
