...
OUT=Path("labs/DEFI_LIQUIDATION_SHOCK_001/SAVE0C_UNIT_METADATA_POPULATION_RECEIPT_V0.2.json")
...
def rpc_slice(targets,offset,length,retries=8):
    ...
    req=urllib.request.Request(RPC,data=raw,headers={"Content-Type":"application/json",
        "User-Agent":"crypto-lab-dls-save0c-unit-completion/0.2"},method="POST")
    last=None
...
targets=[x["reserve"] for x in (agg or {}).get("unmapped_reserves") or []]
...
if not errors and targets:
    for i in range(0,len(targets),100):
        ...
        for j,reserve in enumerate(chunk):
            ...
            recovered[reserve]={"underlying_mint":b58e(da[:32]),"underlying_decimals":int(da[32]),
                                "collateral_mint":b58e(db),"source":"finalized_rpc_dataslice_v0.2"}

# Reconcile only previously unmapped events from frozen partition receipts.
...
for p in sorted(Path(args.partitions).rglob("*.json")):
    ...
    for e in r.get("event_units") or []:
        event_total+=1
        cu=e.get("collateral_underlying")
        if cu:
...
if source_conflicts or event_conflicts:
...
receipt={"schema_version":"0.2","lab_id":"DEFI-LIQUIDATION-SHOCK-001","classification":classification,
 ...
 "target_reserve_count":len(targets),"recovered_reserve_count":len(recovered),
 ...
 "event_total":event_total,"unit_complete_event_count":unit_complete,
 "still_unmapped_event_count":EXPECTED-unit_complete,
 "unique_still_unmapped_reserve_count":len(still_unmapped),
 "still_unmapped_reserves":[{"reserve":k,"event_count":v} for k,v in sorted(still_unmapped.items())],
 ...
 "firewall":{"prices":False,"usd_notional":False,"returns":False,"pnl":False,"direction":False,
             "economic_outcomes":False,"reserve_amounts":False,"token_amounts":False,
