...
-- BIGQUERY TRANSACTION STATUS RECONCILIATION V0.1
-- READ-ONLY / SOURCE-ONLY / OUTCOME-BLIND
--
-- Purpose:
--   Read transaction execution state for the exact 23 frozen Helius-RAW
--   validation signatures from 2024-12-15. This is used only to reconcile
--   BigQuery Transactions.status/err semantics against archival RAW.
--
...
SELECT
  ...
  status,
  err,
  (err IS NULL) AS inferred_success_from_err
