...
C=BASE/"DRIFT_SQD_EVENT_CENSUS_RECEIPT_V0.1.json"
R=BASE/"DRIFT_RAW_SAMPLE_RECONCILIATION_RECEIPT_V0.1.json"
OUT=BASE/"DRIFT_EVENT_CENSUS_FINAL_AUTHORITY_RECEIPT_V0.1.json"
...
class_ok=all(((c.get("classes") or {}).get(k) or {}).get("successful_instruction_count",0)>0 for k in classes)
passed=(c.get("classification")=="DRIFT_SQD_EVENT_CENSUS_COMPLETE_PENDING_RAW_SAMPLE"
        and r.get("classification")=="DRIFT_RAW_SAMPLE_RECONCILIATION_PASS"
        and class_ok
        and r.get("sample_count")==r.get("pass_count")
        and int(r.get("sample_count") or 0)>0)
out={"schema_version":"0.1","lab_id":"DEFI-LIQUIDATION-SHOCK-001",
     "classification":"DRIFT_FOUR_CLASS_EVENT_CENSUS_SOURCE_PASS" if passed else "DRIFT_EVENT_CENSUS_BLOCKED_FAIL_CLOSED",
     "census_classification":c.get("classification"),"raw_sample_classification":r.get("classification"),
     "all_four_classes_observed_successfully":class_ok,
     "classes":c.get("classes"),"raw_sample_count":r.get("sample_count"),"raw_sample_pass_count":r.get("pass_count"),
