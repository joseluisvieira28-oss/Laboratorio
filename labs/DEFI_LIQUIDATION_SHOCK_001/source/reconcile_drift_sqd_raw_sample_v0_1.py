...
OUT=BASE/"DRIFT_RAW_SAMPLE_RECONCILIATION_RECEIPT_V0.1.json"
...
def rpc(sig):
    ...
    req=urllib.request.Request(RPC,data=body,headers={"Content-Type":"application/json","User-Agent":"crypto-lab-dls-drift-raw/0.1"})
    last=None
    for i in range(10):
        try:
...
results=[]; ok=True
for cls,item in (q.get("classes") or {}).items():
    pref=bytes.fromhex(CLASSES[cls])
    for ent in item.get("entries",[]):
        sig=ent["signature"]; obj=rpc(sig); res=obj.get("result")
        rec={"class":cls,"signature":sig,"expected_slot":ent.get("slot"),
...
receipt={"schema_version":"0.1","lab_id":"DEFI-LIQUIDATION-SHOCK-001",
         "classification":"DRIFT_RAW_SAMPLE_RECONCILIATION_PASS" if ok else "DRIFT_RAW_SAMPLE_RECONCILIATION_FAIL_CLOSED",
