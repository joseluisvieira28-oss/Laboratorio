...
BOUNDARY_ISO="2024-07-19T17:54:33Z"
BOUNDARY_TS=int(datetime.fromisoformat(BOUNDARY_ISO.replace("Z","+00:00")).timestamp())
ANCHOR_SIG="5w2cjBXo88QVX3yaBqCrkmd9aHf8ms6MjMyqMXr7J996utdbNJ8pHdd7CeUhva9uFHQYz8fmEKDS89gfvw63aHiT"
ANCHOR_SLOT=307731998
ANCHOR_TS=int(datetime.fromisoformat("2024-12-16T00:00:06+00:00").timestamp())
PAGE_LIMIT=1000
MAX_PAGES=5000
MAX_RETRIES=12
OUT=Path("dls_save11_rpc_boundary_v01")
...
state={
 "schema_version":"0.1","boundary_class":"SAVE11","lab_id":"DEFI-LIQUIDATION-SHOCK-001",
...
for page_no in range(1,MAX_PAGES+1):
    ...
    if not page:
        state.update(phase="BLOCKED",classification="SAVE11_FIRST_SUCCESS_BOUNDARY_RPC_HISTORY_BLOCKED",reason="history_exhausted_before_crossing_lower_boundary")
        write_json("DLS_SAVE11_RPC_FIRST_SUCCESS_RECEIPT_V0.1.json",state)
...
if not state["crossed_lower_boundary"]:
    state.update(phase="BLOCKED",classification="SAVE11_FIRST_SUCCESS_BOUNDARY_RPC_HISTORY_BLOCKED",reason="max_pages_reached_before_lower_boundary")
    write_json("DLS_SAVE11_RPC_FIRST_SUCCESS_RECEIPT_V0.1.json",state)
    print(json.dumps(state,indent=2)); sys.exit(0)
...
for pos,row in enumerate(eligible):
    ...
    res=obj.get("result")
    if res is None:
        state.update(phase="BLOCKED",classification="SAVE11_FIRST_SUCCESS_BOUNDARY_RPC_HISTORY_BLOCKED",
    ...
    if int(res.get("slot",-1))!=int(row["slot"]):
        state.update(phase="FAIL_CLOSED",classification="SAVE11_FIRST_SUCCESS_BOUNDARY_SOURCE_ANOMALY_FAIL_CLOSED",
    ...
    if matched:
        ...
        state.update(phase="COMPLETE",classification="SAVE11_FIRST_SUCCESS_BOUNDARY_RPC_PASS",
...
state.update(phase="COMPLETE_NO_MATCH",classification="SAVE11_FIRST_SUCCESS_BOUNDARY_SOURCE_ANOMALY_FAIL_CLOSED",
             reason="no_liquidation_match_between_source_boundary_and_known_verified_anchor",
