...
EXPANSION COST VALIDATION V0.1

Authority:
- EXPANSION_REPLICATION_FREEZE_V0.1.md
...
SYMBOLS=["XRPUSDT","DOGEUSDT","ADAUSDT","LINKUSDT","AVAXUSDT"]
...
out={
    ...
    "experiment":"EXPANSION_COST_VALIDATION_V0.1",
    "freeze":"EXPANSION_REPLICATION_FREEZE_V0.1",
...
for symbol in SYMBOLS:
    try:
        ...
        for lname,slip in LAYERS.items():
            sym["results"]["PARENT"][lname]=simulate(symbol,bars,funding,"PARENT",slip)
...
if len(valid_symbols)<4:
    out["expansion_gate"]="DATA_BLOCKED"
else:
    for lname in LAYERS:
        rs=[out["symbols"][s]["results"]["PARENT"][lname] for s in valid_symbols]
        out["family"][lname]={
            "adjudicable_asset_count":len(valid_symbols),
            "equal_weight_mean_return":sum(r["net_return"] for r in rs)/len(rs),
            "median_asset_return":sorted(r["net_return"] for r in rs)[len(rs)//2],
    ...
    base=out["family"]["BASE"]
    stress=out["family"]["STRESS"]
    gate=(
        len(valid_symbols)>=4
        and base["positive_assets"]>=3
        and base["pf_gt_1_assets"]>=3
        and stress["equal_weight_mean_return"]>0
        and base["positive_after_remove_top1_assets"]>=2
    )
    out["expansion_gate"]="SURVIVES" if gate else "CROSS_SECTION_EXPANSION_FAIL"

path=EVID/"EXPANSION_COST_VALIDATION_V0.1.json"
path.write_text(json.dumps(out,indent=2),encoding="utf-8")

compact={
    "gate":out["expansion_gate"],
