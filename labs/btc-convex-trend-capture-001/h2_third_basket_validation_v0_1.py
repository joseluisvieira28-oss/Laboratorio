...
H2 THIRD-BASKET VALIDATION V0.1

Authority:
- CHILD_H2_RISING_REGIME_FREEZE.md
...
def load_symbol(symbol):
    ...
    # Frozen expansion rule: exact daily 1h completion for any missing
    # economic-window hour. No interpolation / nearest neighbor.
    end_bar=END_MS
    missing=[t for t in range(START_MS,end_bar+1,3600000) if t not in bd]
    gapfill_dates=sorted(set(datetime.fromtimestamp(t/1000,tz=timezone.utc).strftime("%Y-%m-%d") for t in missing))
    gapfill_added=0
    ...
    economic=[b for b in bars if START_MS<=b["t"]<=END_MS]
    expected=((END_MS-START_MS)//3600000)+1
    if len(economic)!=expected or not economic or economic[0]["t"]!=START_MS or economic[-1]["t"]!=END_MS:
        raise RuntimeError(f"{symbol}: incomplete economic market coverage {len(economic)}/{expected}")

    warmup_start=int(datetime(2020,12,1,0,0,tzinfo=timezone.utc).timestamp()*1000)
    warmup=sum(1 for b in bars if warmup_start<=b["t"]<START_MS)
    if warmup<200:
        raise RuntimeError(f"{symbol}: insufficient warmup {warmup}h")
    ...
    time_meta["warmup_hours_before_boundary"]=warmup
...
def build_signals(bars,mode):
    ...
    for i,b in enumerate(bars):
        if None in (s20[i],sd[i],s200[i],v20[i],r14[i]) or sd[i]==0:
            continue
        ...
        parent_regime=close[i]>s200[i]
        slope_ok=(i>=200 and s200[i-200] is not None and s200[i]>s200[i-200])
        if mode=="H2_RISING_REGIME":
            regime=parent_regime and slope_ok
        else:
            regime=parent_regime
        out[i]=(score>=3 and regime)
        details[i]={
            "z":z,"rsi":r14[i],"score":score,
            "rsi_flag":sr,"z_flag":sz,"bb_flag":sb,"vol_flag":sv,
            "parent_regime":parent_regime,
            "sma200_rising_200h":slope_ok,
            "regime":regime,
        }
    return out,details
...
def simulate(symbol,bars,funding,mode,slip):
    signals,_=build_signals(bars,mode)
...
out={
    ...
    "experiment":"H2_THIRD_BASKET_VALIDATION_V0.1",
    "freeze":"CHILD_H2_RISING_REGIME_FREEZE",
    "source_hierarchy":"AMENDMENTS_004_TO_007_EXACT_SOURCE_RULES",
    ...
    try:
        ...
        sym={
            ...
            "provenance":{
                ...
                "bar_count_economic":sum(1 for b in bars if START_MS<=b["t"]<=END_MS),
                "funding_event_count":sum(1 for t in funding if START_MS<=t<=END_MS),
...
out["valid_symbols"]=valid_symbols
out["blocked_symbols"]=blocked_symbols
out["valid_asset_count"]=len(valid_symbols)
out["blocked_asset_count"]=len(blocked_symbols)

if len(valid_symbols)<4:
    out["h2_gate"]="DATA_BLOCKED"
else:
    for mode in MODES:
        out["family"][mode]={}
        for lname in LAYERS:
            rs=[out["symbols"][s]["results"][mode][lname] for s in valid_symbols]
            vals=sorted(r["net_return"] for r in rs)
            out["family"][mode][lname]={
                "adjudicable_asset_count":len(valid_symbols),
                "equal_weight_mean_return":sum(r["net_return"] for r in rs)/len(rs),
                "median_asset_return":vals[len(vals)//2],
                ...
                "median_mtm_drawdown":sorted(r["max_mark_to_market_drawdown"] for r in rs)[len(rs)//2],
            }

    h2b=out["family"]["H2_RISING_REGIME"]["BASE"]
    h2s=out["family"]["H2_RISING_REGIME"]["STRESS"]
    pb=out["family"]["PARENT"]["BASE"]
    gate=(
        len(valid_symbols)>=4
        and h2b["positive_assets"]>=3
        and h2b["pf_gt_1_assets"]>=3
        and h2s["equal_weight_mean_return"]>0
        and h2b["positive_after_remove_top1_assets"]>=2
        and h2b["equal_weight_mean_return"]>pb["equal_weight_mean_return"]
    )
    out["h2_gate"]="SURVIVES" if gate else "H2_FAIL"

path=EVID/"H2_THIRD_BASKET_VALIDATION_V0.1.json"
...
for s in valid_symbols:
    compact["parent"][s]={
        k:{x:out["symbols"][s]["results"]["PARENT"][k][x] for x in [
            "closed_trades","net_return","cagr","profit_factor",
            "max_mark_to_market_drawdown","funding_cashflow","net_without_top1"
        ]} for k in LAYERS
    }
    compact["h2"][s]={
        k:{x:out["symbols"][s]["results"]["H2_RISING_REGIME"][k][x] for x in [
            "closed_trades","net_return","cagr","profit_factor",
            "max_mark_to_market_drawdown","funding_cashflow","net_without_top1"
        ]} for k in LAYERS
    }
