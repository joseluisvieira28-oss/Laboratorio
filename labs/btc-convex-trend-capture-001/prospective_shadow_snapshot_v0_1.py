#!/usr/bin/env python3
"""
BTC-CONVEX-TREND-CAPTURE-001 — PROSPECTIVE SHADOW SNAPSHOT V0.1

Research-only. Public Binance data only. No orders, no private API, no exchange mutation.
Authority: PROSPECTIVE_SHADOW_FORWARD_FREEZE_V0.1.md
"""
...
FORWARD_START=datetime(2026,9,24,5,0,0,tzinfo=timezone.utc)
...
def main():
    ...
    out={
        ...
        "snapshot":"PROSPECTIVE_SHADOW_SNAPSHOT_V0.1",
        "authority":"PROSPECTIVE_SHADOW_FORWARD_FREEZE_V0.1",
    ...
    readiness={
        "age_ge_90d":age_days>=90,
        "family_closed_trades_ge_40":closed>=40,
        "assets_with_ge_5_trades_ge_3":assets_5>=3,
        "source_coverage_clean":gap_ok,
        "causal_integrity_blocker":False,
    }
    out["family"]={
        "valid_symbols":valid,
        ...
        "adjudication_ready":all([
            readiness["age_ge_90d"],
            readiness["family_closed_trades_ge_40"],
            readiness["assets_with_ge_5_trades_ge_3"],
            readiness["source_coverage_clean"],
            not readiness["causal_integrity_blocker"],
        ]),
    }
    p=EVID/"PROSPECTIVE_SHADOW_SNAPSHOT_V0.1.json"
