#!/usr/bin/env python3
import csv, gzip, hashlib, io, json, math, os, statistics, urllib.parse, urllib.request
...
availability="https://archive.org/wayback/available?"+urllib.parse.urlencode({
    "url":target,
    "timestamp":"20251231235959",
})

receipt={
 "lab_id":"CMM-002-V01",
 "gate":"HOLDOUT_SAFE_STABLECOIN_SOURCE_V01D",
 "outcomes_opened":False,
 "parent_daily_state_sha256":parent_sha,
 "availability_url":availability,
 "remediation_from_v01a":"transport-only: CDX HTTP 503 -> archive.org wayback/available",
 "remediation_from_v01b":"transport-only: decode gzip replay body; same resolved snapshot/source/boundary",
 "remediation_from_v01c":"implementation-only: compare gzip magic using bytes.fromhex(1f8b); no source/science change",
}
try:
    b,_,status=get_bytes(availability,90)
    av=json.loads(b.decode("utf-8"))
    snap=((av.get("archived_snapshots") or {}).get("closest") or {})
    if not snap or not snap.get("available"):
        raise RuntimeError("NO_AVAILABLE_WAYBACK_SNAPSHOT")
    ts=str(snap.get("timestamp") or "")
    if not ts or ts >= "20260101000000":
        raise RuntimeError(f"SNAPSHOT_OUTSIDE_BOUNDARY timestamp={ts}")
    replay=f"https://web.archive.org/web/{ts}id_/{target}"
    rb,_,rstatus=get_bytes(replay,120)
    if rb[:2] == bytes.fromhex("1f8b"):
        rb_decoded = gzip.decompress(rb)
        replay_encoding = "gzip_magic"
    else:
        rb_decoded = rb
        replay_encoding = "identity"
    payload=json.loads(rb_decoded.decode("utf-8"))
    ...
    receipt.update({
      ...
      "archive_payload_transport_sha256":hashlib.sha256(rb).hexdigest(),
      "archive_payload_decoded_sha256":hashlib.sha256(rb_decoded).hexdigest(),
      "archive_replay_encoding":replay_encoding,
...
path=os.path.join(OUT_DIR,"CMM002_STABLECOIN_SOURCE_GATE_V01D.json")
with open(path,"w",encoding="utf-8") as f:
    json.dump(receipt,f,indent=2,sort_keys=True,allow_nan=False)

md=[
 "# CMM-002 — HOLDOUT-SAFE STABLECOIN SOURCE GATE V0.1D",
 "",
...
with open(os.path.join(OUT_DIR,"CMM002_STABLECOIN_SOURCE_GATE_V01D.md"),"w",encoding="utf-8") as f:
    f.write("\n".join(md)+"\n")
