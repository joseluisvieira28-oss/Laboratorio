#!/usr/bin/env python3
"""ARQ-002-CSP-002 full 2024 source census — monthly shard, outcome blind.

Difference from CSP-001:
metrics gaps are persisted into a deterministic source mask rather than causing
the monthly source shard to abort. Conflicting duplicate rows still fail closed.
"""
...
SYMBOL="BTCUSDT"
UA="Crypto-Lab-ARQ002-CSP002-FullSource/0.1"
...
def scan_metrics(day):
    ...
    for t,rs in groups.items():
        ...
        observed.append(t); exact_removed+=len(rs)-1
    ts=sorted(observed)
    lo=int(datetime(day.year,day.month,day.day,tzinfo=timezone.utc).timestamp()*1000)
    exp=[lo+i*300000 for i in range(288)]
    exp_set=set(exp)
    if any(t not in exp_set for t in ts): raise GateError(f"METRICS_OFF_GRID:{ds}")
    if ts and ts[-1]>=PROTECTED_MS: raise GateError(f"METRICS_PROTECTED:{ds}")
    missing=sorted(exp_set-set(ts))
    return {
      "date":ds,"observed_rows":len(ts),"sha256":sha,
      "exact_dupes_removed":exact_removed,
      "missing_count":len(missing),
      "missing_timestamps_utc":[iso(t) for t in missing],
...
def warmup():
    d=date(2023,12,31)
    return {"kline":scan_kline(d),"metrics":scan_metrics(d),"funding":scan_funding(2023,12),"pass":True}
...
def main():
    ...
    rec={"lab_id":"ARQ-002-CSP-002","gate":"FULL_SOURCE_CENSUS_MONTH_V0.1","month":args.month,
    ...
    rec["metrics_observed_slots"]=sum(x["metrics"]["observed_rows"] for x in rec["days"]) if rec["days"] else 0
    rec["metrics_missing_slots"]=sum(x["metrics"]["missing_count"] for x in rec["days"]) if rec["days"] else 0
    rec["metrics_missing_timestamps_utc"]=[
      ts for x in rec["days"] for ts in x["metrics"]["missing_timestamps_utc"]
    ]
    ...
    out=Path(f"arq002_csp002_source_{args.month}.json")
    out.write_text(json.dumps(rec,indent=2,sort_keys=True)+"\n",encoding="utf-8")
    print(json.dumps({"month":args.month,"classification":rec["classification"],"days_count":rec["days_count"],
                      "agg_rows_total":rec["agg_rows_total"],"metrics_missing_slots":rec["metrics_missing_slots"],
