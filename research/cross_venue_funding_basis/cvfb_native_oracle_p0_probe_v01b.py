...
def is_valid_block_response(data: object) -> bool:
    if not isinstance(data, dict):
        return False
    if data.get("type") != "blockDetails":
        return False
    d = data.get("blockDetails")
    ...
    for attempt in range(retries):
        ...
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "CryptoLab-CVFB-P0/0.1B",
            },
            method="POST",
        )
        try:
            with urllib.request.urlopen(req, timeout=30) as r:
                ...
                if not is_valid_block_response(data):
                    raise RuntimeError("semantic-invalid blockDetails response")
                return data
            ...
            last = e
            if attempt + 1 < retries:
                time.sleep(min(65.0, max(2.3, 2.0 ** attempt)))
    fail(f"blockDetails height={height} unavailable after semantic-validating retries: {type(last).__name__ if last else 'unknown'}")


def details(raw: dict) -> dict:
    d = raw.get("blockDetails")
    if not isinstance(d, dict):
...
def block_time(raw: dict) -> int:
    try:
        return int(details(raw)["blockTime"])
    except Exception:
