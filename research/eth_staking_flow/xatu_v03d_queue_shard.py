...
def main():
    ...
    if not(GLOBAL_START<=start<=end<=GLOBAL_END):raise SystemExit("invalid shard")
    out=Path("xatu_v03d_queue_shards");out.mkdir(parents=True,exist_ok=True)
    dst=out/f"queue_{sid}.json"
    rows=[];errors=[];d=start
    while d<=end:
    ...
    receipt={"lab_id":LAB_ID,"mve_id":MVE_ID,"phase":"XATU_FULL_QUEUE_RECONSTRUCTION_SHARD_V0_3D",
    ...
    print(json.dumps({"shard":sid,"classification":cls,"observed":len(rows),"errors":len(errors),"validator_level_rows_persisted":False,"signal":False,"returns":False,"pnl":False},sort_keys=True))
