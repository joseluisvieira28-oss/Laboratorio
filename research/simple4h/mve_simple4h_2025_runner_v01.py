...
BOOT_REPS=3000
BOOT_BLOCK=5
SEED_TEXT="MVE_SIMPLE4H_01_2025_V01"
ARM_TOKEN="MVE_SIMPLE4H_01_2025_ONE_SHOT"
...
def summarize(ev,cid,asset,strategy):
    ...
    pos=x[x>0]; pos_sum=float(pos.sum())
    ...
    r.update({'gross_mean_bps':float(ev.gross_bps.mean()),'net10_mean_bps':mean,'net10_median_bps':float(ev.net10_bps.median()),'net14_mean_bps':float(ev.net14_bps.mean()),'win_rate_net10':float((ev.net10_bps>0).mean()),'sd_net10_bps':sd,'se_net10_bps':se,'ci95_low_net10_bps':lo,'ci95_high_net10_bps':hi,'t_stat_net10':t,'p_one_sided_net10':p,'max_drawdown_net10_pct':maxdd(x),'mean_r_multiple':float(ev.r_multiple.mean()),'stop_rate':float(reasons.str.startswith('STOP').mean()),'target_rate':float((reasons=='TARGET').mean()),'time_exit_rate':float((reasons=='TIME').mean()),'q1_net10_mean_bps':qs[0],'q2_net10_mean_bps':qs[1],'q3_net10_mean_bps':qs[2],'q4_net10_mean_bps':qs[3],'positive_quarters':int(sum(np.isfinite(v) and v>0 for v in qs)),'top_trade_positive_share':top_trade_share,'top_quarter_positive_share':top_q_share})
    return r

def classify(summary):
    valid100=int((summary.n>=100).sum()); n50=int((summary.n>=50).sum()); pos14=int(((summary.n>=50)&(summary.net14_mean_bps>0)).sum()); strong=int(summary.strong_cell_pass.sum()); strat=int(summary.loc[summary.strong_cell_pass,'strategy'].nunique()); med14=float(summary.net14_mean_bps.median()) if len(summary) else np.nan
    if strong>=2 and strat>=2 and med14>0 and not summary.loc[summary.strong_cell_pass,'concentration_red_flag'].any(): decision='TIER_2_PROMOTED_CANDIDATE'
    elif strong==1 or pos14>=4: decision='TIER_3_WATCHLIST'
    elif valid100>=5 and strong==0 and med14<=0: decision='REJECTED_STONE'
    elif valid100>=5 and strong==0: decision='NO_EDGE'
    else: decision='INSUFFICIENT_SAMPLE'
    return decision,{'valid_n_ge_100_cells':valid100,'n_ge_50_cells':n50,'positive_net14_cells':pos14,'strong_cell_pass_count':strong,'strong_cell_pass_strategy_count':strat,'median_net14_bps':med14}
...
def self_test():
    ...
    print(json.dumps({'status':'SELF_TEST_PASS','protected_market_data_read':False,'2026_opened':False,'live_trading_authorized':False},indent=2))
...
def main():
    ...
    summary['strong_cell_pass']=(summary.n>=100)&(summary.net10_mean_bps>0)&(summary.net14_mean_bps>0)&(summary.q_bh_7<0.05)&(summary.bootstrap_lower95_net10_bps>0)&(summary.positive_quarters>=3)
    ...
    receipt={'mve_id':MVE_ID,'run_type':'ONE_SHOT_2025_CONFIRMATION','branch_head':auth['branch_head'],'source_manifest_sha256':manifest['manifest_sha256'],'period':'2025-01-01T00:00:00Z/2026-01-01T00:00:00Z','family_metrics':fm,'decision':decision,'cells':summary.to_dict('records'),'2026_opened':False,'live_trading_authorized':False}
    (out/'MVE_SIMPLE4H_01_2025_DECISION_RECEIPT.json').write_text(json.dumps(receipt,indent=2,default=str),encoding='utf-8')
    print(json.dumps({'status':'ONE_SHOT_COMPLETE','decision':decision,'family_metrics':fm,'2026_opened':False,'live_trading_authorized':False},indent=2))
