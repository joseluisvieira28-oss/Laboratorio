...
Research-only. Computes contemporaneous state; never inspects future outcomes.
"""
...
class L2Replay:
    ...
    def apply(self, message: dict, compute_depth_features: bool=True) -> BookState:
        ...
        mid=(bb+ba)/2.0
        bid_q=self.bids[bb]; ask_q=self.asks[ba]
        denom=bid_q+ask_q
        micro=(ba*bid_q + bb*ask_q)/denom if denom else mid
        l1=(bid_q-ask_q)/denom if denom else 0.0

        if compute_depth_features:
            top_b=heapq.nlargest(10,self.bids.items(),key=lambda x:x[0])
            top_a=heapq.nsmallest(10,self.asks.items(),key=lambda x:x[0])
            def imb(n):
                bq=sum(q for _,q in top_b[:n])
                aq=sum(q for _,q in top_a[:n])
                tot=bq+aq
                return (bq-aq)/tot if tot else 0.0
            l5=imb(5); l10=imb(10)
        else:
            l5=math.nan; l10=math.nan
