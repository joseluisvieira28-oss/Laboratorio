...
OUT=Path(f"artifacts/cclm002_discovery_flow_month_{MONTH}.json")
...
def decode_received(log):
    ...
    return {"source_domain":source,"nonce":int(ts[2],16),"sender":sender.lower(),"body":body,
            "tx":str(log.get("transactionHash") or "").lower(),"log_index":int(li),"block_number":int(bn)}
...
receipt={"lab_id":"CROSSCHAIN-LIQUIDITY-MIGRATION-001","child_id":"CCLM-CCTP-SETTLED-FLOW-002",
 "stage":"DISCOVERY_PREDICTOR_FLOW_MONTH_SOURCE_V0.1","window_month":MONTH,
    ...
    for dd in (0,1):
        ...
        for l in rec:
            ...
            canonical.append({"source_domain":sd,"destination_domain":dd,"nonce":x["nonce"],"amount_atomic":b["amount"],
                              "destination_block_number":x["block_number"],
                              "body_sha256":hashlib.sha256(x["body"]).hexdigest(),
                              "mint_log_index":chosen["log_index"],"received_log_index":x["log_index"]})
        # Resolve destination block timestamps after semantic pairing. No market data.
        ts_cache={}
        for ev in canonical:
            bn=ev["destination_block_number"]
            if bn not in ts_cache:
                ts_cache[bn]=block_ts(block(c["boundary_rpc"],bn))
            ev["settled_ts_s"]=ts_cache[bn]
            ev["signed_amount_atomic"]=ev["amount_atomic"] if sd==0 and dd==1 else -ev["amount_atomic"]
        ...
        receipt["routes"].append({"source_domain":sd,"destination_domain":dd,"received_logs_total":len(rec),
          ...
          "canonical_events":canonical,"evidence":[evr,evm]})
    receipt["canonical_settled_flows_total"]=total
    receipt["semantic_mismatch_count"]=mismatches
    receipt["classification"]="DISCOVERY_FLOW_MONTH_SOURCE_PASS" if mismatches==0 else "DISCOVERY_FLOW_MONTH_SOURCE_BLOCKED"
except Exception as e:
    receipt["classification"]="DISCOVERY_FLOW_MONTH_TECHNICAL_FAILURE";receipt["error"]=f"{type(e).__name__}:{str(e)[:800]}"
