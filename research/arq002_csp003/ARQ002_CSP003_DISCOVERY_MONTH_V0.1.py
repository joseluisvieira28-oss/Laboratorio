#!/usr/bin/env python3
"""ARQ-002-CSP-003 frozen Discovery monthly shard.
...
UTC=timezone.utc
SOURCE_CLOSEOUT=Path("research/arq002_csp003/ARQ002_CSP003_SOURCE_CLOSEOUT_V0.1.json")
COST_LOW=0.0010
COST_BASE=0.0014
COST_STRESS=0.0020
...
def load_metrics_day(d):
    ...
    for t,rs in groups.items():
        ...
        try:
            ...
            continue
        # CSP-003 frozen validity mask: present but non-finite/non-positive OI
        # is UNAVAILABLE_SOURCE, exactly like a missing slot. Never repair it.
...
def source_closeout_guard():
    ...
    if r.get("lab_id")!="ARQ-002-CSP-003" or r.get("classification")!="SOURCE_CENSUS_PASS":
        raise FrozenError("SOURCE_CLOSEOUT_NOT_PASS")
    if r.get("economic_values_opened_for_csp003") is not False or r.get("outcomes_opened_for_csp003") is not False:
        raise FrozenError("SOURCE_CLOSEOUT_FIREWALL")
    if r.get("csp002_partial_outputs_used") is not False:
        raise FrozenError("CSP002_CONTAMINATION_FIREWALL")
    return r
...
def main():
    ...
    if int(frozen_mask.get("unavailable_count",-1))!=len(actual_missing) or frozen_mask.get("sha256")!=actual_mask_sha:
        raise FrozenError(
    ...
    for d in days:
        ...
        for i in range(1440):
            ...
            # CSP-003 validity mask: exact nominal slots required; never stale fallback.
    ...
    out=Path(f"arq002_csp003_discovery_{args.month}.json")
