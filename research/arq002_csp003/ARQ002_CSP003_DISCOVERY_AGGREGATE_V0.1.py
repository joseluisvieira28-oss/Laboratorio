#!/usr/bin/env python3
"""ARQ-002-CSP-003 frozen 2024 Discovery aggregate."""
...
REPS=10000
SEED=2002001
OUT=Path("ARQ002_CSP003_DISCOVERY_RECEIPT_V0.1.json")

def mean(xs): return float(statistics.fmean(xs)) if xs else None

files=sorted(Path("monthly_discovery").glob("arq002_csp003_discovery_2024-*.json"))
rec={
  "lab_id":"ARQ-002-CSP-003","classification":"RUNNING",
...
try:
    ...
    for p in files:
        ...
        masked+=int(r["source_validity_masked_sweep_events"])
    ...
    rec.update({
      ...
      "source_validity_masked_sweep_events":masked,"ambiguous_both_sides":ambig,"no_cvd_notional_events":nocvd,
