...
const prep=JSON.parse(await readFile(DIR+'/lcod_active_population_prepare_v07.json','utf8'))
const fs=files.filter(x=>/^lcod_active_population_chunk_\d+_v07\.json$/.test(x)).sort()
...
const out={lab_id:'LIQUIDATION-CONVEXITY-ORACLE-DISTANCE-001',stage:'BLOCK_PINNED_ACTIVE_POPULATION_V0.7',
...
await mkdir('artifacts',{recursive:true});await writeFile('artifacts/lcod_block_pinned_active_population_v07.json',JSON.stringify(out,null,2)+'\n')
