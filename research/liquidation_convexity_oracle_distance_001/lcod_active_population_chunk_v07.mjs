...
async function rpc(payload,retries=5){
 for(let a=0;a<retries;a++){
  try{
   const r=await fetch(RPC,{method:'POST',headers:{'content-type':'application/json','user-agent':'CryptoLab-LCOD-ActiveChunk/0.7'},body:JSON.stringify(payload)})
...
for(let off=0;off<pairList.length;off+=80){
 const part=pairList.slice(off,off+80)
 ...
 if(!Array.isArray(arr)){errors.push('NON_ARRAY_BATCH');break}
 const m=new Map(arr.map(x=>[x.id,x]))
 for(let i=0;i<part.length;i++){
  const p=part[i],x=m.get(off+i+1)
  if(!x||x.error||typeof x.result!=='string'){errors.push(sha(p)+':RPC');continue}
  try{
   const h=x.result.replace(/^0x/,'');if(h.length<64*7)throw new Error('SHORT')
   const debt=BigInt('0x'+h.slice(64*4,64*5))
   ;(debt>0n?active:inactive).push(sha(p))
  }catch{errors.push(sha(p)+':DECODE')}
 }
 await sleep(40)
}
...
const out={stage:'BLOCK_PINNED_ACTIVE_POPULATION_CHUNK_V0.7',chunk_id:CID,from_block:FROM,to_block:TO,
...
await mkdir('artifacts',{recursive:true});await writeFile(`artifacts/lcod_active_population_chunk_${String(CID).padStart(2,'0')}_v07.json`,JSON.stringify(out,null,2)+'\n')
