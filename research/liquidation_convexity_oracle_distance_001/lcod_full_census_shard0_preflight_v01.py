...
OUT=Path("artifacts/lcod_full_census_shard0_preflight_v01.json")
SHARD="0"
TOL=5e-5
...
def rpc(method,params):
    ...
    for attempt in range(4):
        try:
            req=urllib.request.Request(URL,data=payload,headers={"Content-Type":"application/json","User-Agent":"CryptoLab-LCOD-001-Shard0/0.1"},method="POST")
...
shard=sorted(w for w in all_wallets if hashlib.sha256(w.encode()).hexdigest().startswith(SHARD))
detail_cache={}
def detail(rid):
...
receipt={
 ...
 "stage":"FULL_CENSUS_SHARD0_ENGINEERING_PREFLIGHT_V0.1",
 "captured_at_utc":datetime.now(timezone.utc).isoformat(),
 "shard_rule":"sha256(lowercase_wallet)[0] == '0'",
 "full_index_borrowers":len(all_wallets),
 "shard_indexed_borrowers":len(shard),
 ...
 "classification":"SHARD0_SOURCE_PREFLIGHT_PASS" if passed else "SHARD0_SOURCE_PREFLIGHT_BLOCKED",
...
print(json.dumps({k:receipt[k] for k in ("classification","full_index_borrowers","shard_indexed_borrowers","debt_bearing_borrowers","eligible_borrowers","eligible_coverage","max_observed_relative_hf_error","exclusion_reasons","census_errors","read_errors")},indent=2))
