#!/usr/bin/env python3
"""2024 replication reserve shard: token-native replay + daily point-in-time valuation."""
        ...
        try:
            r=requests.post(PORTAL,json=body,timeout=(20,180),stream=True,
                headers={"Content-Type":"application/json","Accept-Encoding":"gzip",
                         "User-Agent":f"{LAB_ID}/replication-reserve-v0.1"})
...
def rpc_batch(endpoint:str,items:list[dict[str,Any]],stats:Counter[str])->dict[int,int]:
    ...
    headers={"Content-Type":"application/json","User-Agent":f"{LAB_ID}/replication-reserve-v0.1"}
...
def main()->int:
    sid=int(os.environ.get("REPLICATION_RESERVE_SHARD_ID","0"))
    scount=int(os.environ.get("REPLICATION_RESERVE_SHARD_COUNT","8"))
    out=Path("replication_reserve_shards"); out.mkdir(parents=True,exist_ok=True)
    meta_path=out/f"replication_reserve_shard_{sid:02d}_meta.json"
    rows_path=out/f"replication_reserve_shard_{sid:02d}_rows.jsonl.gz"
    ...
    try:
        ...
        if len(snaps)!=366: raise RuntimeError("calendar snapshot count mismatch")
        ...
        if not selected: raise RuntimeError("empty active 2024 reserve shard")
        ...
        # frozen 2024 state ceiling are canonical master identities but cannot
        # contribute state in this partition and therefore do not require a
        # pre-existing aToken Initialized/decimals event inside 2024.
        ...
        meta={
            "lab_id":LAB_ID,
            "classification":"REPLICATION_RESERVE_SHARD_PASS",
            "shard_id":sid,"shard_count":scount,
            "selected_reserves":underlyings,
            "assigned_reserves_full_r1":[u for u,_m in selected_all],
            "post_2024_reserves_excluded":[u for u,m in selected_all if int(m["init_block"])>to_block],
            ...
            "safety":{
                ...
                "future_liquidation_outcomes_opened":False,"opened_2024_predictor":False,"opened_2024_outcomes":False,
    ...
    return 0 if meta["classification"]=="REPLICATION_RESERVE_SHARD_PASS" else 2
