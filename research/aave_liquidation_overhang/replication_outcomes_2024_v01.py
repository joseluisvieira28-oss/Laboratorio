#!/usr/bin/env python3
"""Open only the frozen 2024 Aave replication LiquidationCall outcomes after predictor persistence, with 2024-12-31 right-censored by the 2025 firewall."""
        ...
        try:
            r=requests.post(PORTAL,json=body,timeout=(20,180),stream=True,
                headers={"Content-Type":"application/json","Accept-Encoding":"gzip",
                         "User-Agent":f"{LAB_ID}/replication-outcomes-v0.1"})
...
def rpc_batch(endpoint:str,items:list[dict[str,Any]],stats:Counter[str])->dict[int,int]:
    ...
    headers={"Content-Type":"application/json","User-Agent":f"{LAB_ID}/replication-outcomes-v0.1"}
...
def main()->int:
    out=Path("replication_outcome_output"); out.mkdir(parents=True,exist_ok=True)
    dst=out/"AAVE_LIQUIDATION_OVERHANG_001_REPLICATION_OUTCOME_2024_V0_1.json"
    ...
    try:
        ...
        if len(days)!=366 or len(snaps)!=366: raise RuntimeError("predictor/calendar count mismatch")
        ...
        start_block=blocks[0]; end_block=int(cal["replication_outcome_to_block"])
        ...
        for obj in stream(start_block,end_block,transport):
            ...
            if ts>=1735689600: raise RuntimeError("2025 liquidation outcome opened")
        ...
        receipt={
            "lab_id":LAB_ID,
            "classification":"REPLICATION_OUTCOME_PASS",
            "predictor_sha256":pred["predictor_sha256"],
            "snapshot_count":len(daily),
            "predictor_snapshot_count":len(days),
        ...
        klass="REPLICATION_PROVENANCE_FAILURE" if "disagreement" in str(exc) or "outside canonical" in str(exc) or "2025" in str(exc) else "REPLICATION_ACQUISITION_TECHNICAL_FAILURE"
        receipt={
    ...
    print(json.dumps({"classification":receipt["classification"],
                      ...
                      "opened_2024_outcomes":True,"market_returns_opened":False,"pnl_opened":False},sort_keys=True))
    return 0 if receipt["classification"]=="REPLICATION_OUTCOME_PASS" else 2
