...
PHASE = "DISCOVERY_D0_GLOBAL_SOURCE_AND_SNAPSHOT_MAPPING_V0_1_1"
...
MAP_HI_BLOCK = 19_007_945\nMAX_BRACKET_TS = int(datetime(2024, 1, 2, 0, 0, 0, tzinfo=timezone.utc).timestamp())
...
def headers_batch(endpoint: str, blocks: list[int], stats: Counter[str]) -> dict[int, dict[str, Any]]:
    ...
    for start in range(0, len(unique), 24):
        ...
        for b, h in zip(chunk, vals):
            ...
            if ts > MAX_BRACKET_TS:
                raise RuntimeError("header exceeds frozen bracketing ceiling")
            out[b] = h
    return out
