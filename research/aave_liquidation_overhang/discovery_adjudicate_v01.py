#!/usr/bin/env python3
"""Canonical 2023 Discovery adjudicator for AAVE-LIQUIDATION-OVERHANG-001."""
...
def main()->int:
    out=Path("discovery_canonical_output"); out.mkdir(parents=True,exist_ok=True)
    dst=out/"AAVE_LIQUIDATION_OVERHANG_001_DISCOVERY_2023_CANONICAL_V0_1.json"
    receipt={}
    try:
        pred=load_one("downloaded_discovery_predictor","DISCOVERY_PREDICTOR_PASS")
        outcome=load_one("downloaded_discovery_outcome","DISCOVERY_OUTCOME_PASS")
        p=pred["daily_predictor"]; o=outcome["daily_outcome"]
        if len(p)!=len(o): raise RuntimeError("predictor/outcome row count mismatch")
        ...
        if any(not str(x["date"]).startswith("2023-") for x in p):
            raise RuntimeError("non-2023 predictor row opened")
        ...
        eligible=n>=300 and overhang_pos>=30 and outcome_pos>=20
        ...
        if not eligible:
            ...
            classification="DISCOVERY_NO_SIGNAL"; failure="primary Spearman rho undefined"
        else:
            gates={
                "rho_positive":rho>0,
            ...
            classification="DISCOVERY_MECHANISM_PASS" if all(gates.values()) else "DISCOVERY_NO_SIGNAL"
            failure=None if classification=="DISCOVERY_MECHANISM_PASS" else "one or more frozen Discovery signal gates failed"

        gates={
            "snapshot_count_ge_300":n>=300,
        ...
        receipt={
            ...
            "gates":gates,
            "next_authorized_phase":"2024_REPLICATION_PROTOCOL_EXECUTION" if classification=="DISCOVERY_MECHANISM_PASS" else None,
            "mechanism_only":True,
        ...
        receipt={
            "lab_id":LAB_ID,
            "classification":"DISCOVERY_PROVENANCE_FAILURE",
    ...
    return 0 if receipt["classification"] in ("DISCOVERY_MECHANISM_PASS","DISCOVERY_NO_SIGNAL","DISCOVERY_INSUFFICIENT_SAMPLE") else 2
