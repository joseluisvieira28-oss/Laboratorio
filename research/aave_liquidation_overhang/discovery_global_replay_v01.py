#!/usr/bin/env python3
"""2023-only global/eMode/oracle replay for AAVE-LIQUIDATION-OVERHANG-001 Discovery."""
        ...
        try:
            r=requests.post(PORTAL,json=body,timeout=(20,180),stream=True,
                headers={"Content-Type":"application/json","Accept-Encoding":"gzip",
                         "User-Agent":f"{LAB_ID}/discovery-global-v0.1"})
...
def rpc_batch(endpoint:str,items:list[dict[str,Any]],stats:Counter[str])->dict[int,str]:
    ...
    headers={"Content-Type":"application/json","User-Agent":f"{LAB_ID}/discovery-global-v0.1"}
...
def main()->int:
    out=Path("discovery_global_output"); out.mkdir(parents=True,exist_ok=True)
    dst=out/"AAVE_LIQUIDATION_OVERHANG_001_DISCOVERY_GLOBAL_V0_1.json"
    transport=Counter(); rpcstats={ep:Counter() for ep in RPC_ENDPOINTS}
    try:
        cal_files=list(Path("downloaded_discovery_calendar").rglob("*.json"))
        if len(cal_files)!=1: raise RuntimeError("expected exactly one calendar receipt")
        cal=json.loads(cal_files[0].read_text())
        if cal.get("classification")!="DISCOVERY_CALENDAR_PASS": raise RuntimeError("calendar not PASS")
        ...
        receipt={
            "lab_id":LAB_ID,
            "classification":"DISCOVERY_GLOBAL_SOURCE_PASS",
            ...
            "safety":{
                ...
                "opened_2024_outcomes":False,"opened_2025_or_2026":False,
                "market_returns_opened":False,"pnl_opened":False,
                "live_trading":False,"exchange_mutation":False,
            },
        }
    except Exception as exc:
        klass="DISCOVERY_PROVENANCE_FAILURE" if "stable/non-variable" in str(exc) or "escaped frozen" in str(exc) or "category price source" in str(exc) or "oracle replay mismatch" in str(exc) or "disagreement" in str(exc) else "DISCOVERY_ACQUISITION_TECHNICAL_FAILURE"
        receipt={
    ...
    return 0 if receipt["classification"]=="DISCOVERY_GLOBAL_SOURCE_PASS" else 2
