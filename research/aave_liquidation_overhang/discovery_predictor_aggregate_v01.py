#!/usr/bin/env python3
"""Aggregate 8 reserve shards into the frozen 2023 Aave liquidation-overhang predictor."""
...
def main()->int:
    out=Path("discovery_predictor_output"); out.mkdir(parents=True,exist_ok=True)
    dst=out/"AAVE_LIQUIDATION_OVERHANG_001_DISCOVERY_PREDICTOR_2023_V0_1.json"
    receipt={}
    try:
        ...
        snaps=cal["snapshots"]
        if len(snaps)!=334: raise RuntimeError("snapshot count mismatch")

        meta_files=sorted(Path("downloaded_discovery_reserves").rglob("reserve_shard_*_meta.json"))
        row_files=sorted(Path("downloaded_discovery_reserves").rglob("reserve_shard_*_rows.jsonl.gz"))
        ...
        if any(x.get("classification")!="DISCOVERY_RESERVE_SHARD_PASS" for x in metas):
        ...
        if len(daily)!=334: raise RuntimeError("daily predictor count mismatch")

        receipt={
            "lab_id":LAB_ID,
            "classification":"DISCOVERY_PREDICTOR_PASS",
    ...
    return 0 if receipt["classification"]=="DISCOVERY_PREDICTOR_PASS" else 2
