...
echo ============================================================
echo TFG-VWAP-15M-001 SOURCE BYTE GATE V0.2
echo OUTCOME-BLIND / NO PNL / NO 2024 / NO 2025 / NO 2026
echo ============================================================

if not "%~1"=="" call :ACCEPT_DIR "%~1"
...
if not defined NORMALIZED_DIR (
  for %%D in (
    ...
    if exist "%%~D" (
      for /f "usebackq delims=" %%F in (`powershell -NoProfile -Command "$roots=Get-ChildItem -LiteralPath '%%~D' -Filter '%REF_PACKAGE%' -File -Recurse -Force -ErrorAction SilentlyContinue ^| Select-Object -ExpandProperty DirectoryName; foreach($d in $roots){$ok=$true; foreach($s in @('BTCUSDT','ETHUSDT','SOLUSDT','BNBUSDT','XRPUSDT','DOGEUSDT')){if(-not (Test-Path -LiteralPath (Join-Path $d ('H180-0001_NORMALIZED_'+$s+'.zip')) -PathType Leaf)){$ok=$false;break}}; if($ok){$d; break}}"`) do (
...
if not defined NORMALIZED_DIR (
  ...
  echo Reference package: %REF_PACKAGE%
...
for %%S in (BTCUSDT ETHUSDT SOLUSDT BNBUSDT XRPUSDT DOGEUSDT) do (
  if not exist "%CANDIDATE%\H180-0001_NORMALIZED_%%S.zip" exit /b 0
)
set "NORMALIZED_DIR=%CANDIDATE%"
exit /b 0
