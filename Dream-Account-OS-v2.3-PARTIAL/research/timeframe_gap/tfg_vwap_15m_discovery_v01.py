...
"""Frozen Discovery runner for TFG-VWAP-15M-001.
...
def load_authority(binding_path: Path) -> dict:
    ...
    if binding.get("status") != "FROZEN_BEFORE_ANY_VWAP_15M_OUTCOME_EVALUATION":
    ...
    frozen_hashes = binding.get("normalized_package_sha256_discovery_only", {})
    if frozen_hashes != EXPECTED_PACKAGE_SHA256:
...
def load_symbol(normalized_dir: Path, symbol: str) -> tuple[pd.DataFrame, dict]:
    ...
    with zipfile.ZipFile(package, "r") as archive:
        ...
        unexpected_normalized = [n for n in names if n.endswith(".normalized.csv.zst") and n not in required]
        if unexpected_normalized:
            raise RuntimeError(f"UNEXPECTED_NORMALIZED_MEMBER:{symbol}:{unexpected_normalized[0]}")
...
def minute_vwap_and_15m(df: pd.DataFrame) -> pd.DataFrame:
    ...
    q = d.resample("15min", label="left", closed="left").agg(
        open=("open", "first"),
        high=("high", "max"),
        low=("low", "min"),
        close=("close", "last"),
...
def summarize(trades: pd.DataFrame) -> dict:
    ...
    quarterly = {
        quarter: {"n": int(len(g)), "net10_mean_bps": float(g["net10_bps"].mean())}
        for quarter, g in t.assign(quarter=q).groupby("quarter")
    }
    return {
        "n": int(len(t)),
...
def execute(normalized_dir: Path, binding_path: Path, output_dir: Path) -> dict:
    ...
    for symbol in SYMBOLS:
        ...
        bars = minute_vwap_and_15m(minute)
        trades, signal_audit = make_trades(bars, symbol)
        source_audits.append(source_audit)
        signal_audits[symbol] = {
            **signal_audit,
            "derived_15m_bar_count": int(len(bars)),
            "valid_15m_bar_count": int(bars["bar_ok"].sum()),
        }
        all_trades.append(trades)
        print(
            f"DISCOVERY {symbol}: minute_rows={len(minute):,} valid_15m={int(bars['bar_ok'].sum()):,} trades={len(trades):,}",
            flush=True,
        )
    ...
    trades_path = output_dir / "TFG_VWAP_15M_001_DISCOVERY_TRADES_V0.1.csv"
    trades.to_csv(trades_path, index=False)
    receipt = {
        "status": "TFG_VWAP_15M_DISCOVERY_COMPLETE",
        "lab_id": LAB_ID,
        "stage": "DISCOVERY",
    ...
    receipt_path = output_dir / "TFG_VWAP_15M_001_DISCOVERY_RECEIPT_V0.1.json"
    receipt_path.write_text(json.dumps(receipt, sort_keys=True, indent=2, default=str) + "\n", encoding="utf-8")
    return receipt
...
def self_test() -> None:
    ...
    bars = minute_vwap_and_15m(df)
    assert len(bars) == 24 * 4 * 4
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Frozen TFG-VWAP-15M-001 Discovery runner")
    ...
    try:
        ...
        if args.output_dir is not None:
            ...
            (args.output_dir / "TFG_VWAP_15M_001_DISCOVERY_BLOCKED_V0.1.json").write_text(
