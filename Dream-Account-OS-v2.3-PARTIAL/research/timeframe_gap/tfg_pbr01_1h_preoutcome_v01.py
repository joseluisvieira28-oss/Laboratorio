...
"""Outcome-blind PBR01 1H preparation engine.

This module may aggregate the already-authorized MEXC 15m Discovery corpus into
strict UTC 1H bars and form the frozen PBR01 geometry. It deliberately contains
NO outcome simulator, PnL evaluator, bootstrap, validation unlock, network code,
or exchange action. TFG-PBR01-1H-001 remains activation-blocked by its freeze.
"""
...
SETUP_ID = "PBR01_BREAKOUT_RETEST_LONG__TFG_1H"
...
class ResearchParameters1H:
    ...
    def validate(self) -> None:
        if self.timeframe != RESEARCH_TIMEFRAME:
            raise ValueError("TFG-PBR01-1H-001 is frozen to 1h")
        for name, value in {
...
def validate_1h_candles(candles: Iterable[Candle], *, require_regular_spacing: bool = True) -> list[Candle]:
    series = [c for c in candles if c.closed]
    previous_open: int | None = None
    previous_close: int | None = None
    for candle in series:
        _validate_ohlcv(candle)
        if candle.open_time % ONE_H_MS != 0:
            raise ValueError("1h candle is not UTC aligned")
        if candle.close_time != candle.open_time + ONE_H_MS - 1:
            raise ValueError("1h close_time mismatch")
        if previous_open is not None:
            if candle.open_time <= previous_open or candle.close_time <= (previous_close or -1):
                raise ValueError("timestamps must be strictly increasing")
            if require_regular_spacing and candle.open_time - previous_open != ONE_H_MS:
                raise ValueError("1h series contains a gap")
        previous_open = candle.open_time
        previous_close = candle.close_time
    return series


def aggregate_15m_to_1h(candles: Iterable[Candle]) -> list[Candle]:
    """Create only complete 1H UTC buckets; incomplete buckets are dropped."""
    ...
    for candle in source:
        ...
        bucket_start = candle.open_time - candle.open_time % ONE_H_MS
        buckets.setdefault(bucket_start, []).append(candle)

    output: list[Candle] = []
    for bucket_start in sorted(buckets):
        items = sorted(buckets[bucket_start], key=lambda c: c.open_time)
        expected = [bucket_start + i * FIFTEEN_MIN_MS for i in range(CANDLES_PER_1H)]
        if len(items) != CANDLES_PER_1H or [c.open_time for c in items] != expected:
            continue
        output.append(Candle(
            ...
            bucket_start + ONE_H_MS - 1,
            True,
        ))
    return validate_1h_candles(output, require_regular_spacing=False)


def split_1h_segments(candles: Iterable[Candle]) -> tuple[list[list[Candle]], int]:
    series = validate_1h_candles(candles, require_regular_spacing=False)
    if not series:
        return [], 0
    segments: list[list[Candle]] = [[series[0]]]
    gaps = 0
    for candle in series[1:]:
        if candle.open_time - segments[-1][-1].open_time == ONE_H_MS:
...
def derive_signal_geometries_1h(
    candles: Iterable[Candle],
    parameters: ResearchParameters1H,
    costs: CostAssumptions,
) -> list[SignalGeometry]:
    """Form frozen PBR01 geometry only. No trade outcome is inspected here."""
    parameters.validate()
    costs.validate()
    series = validate_1h_candles(candles, require_regular_spacing=True)
    ...
    for breakout_index in range(minimum_history, len(series) - 2):
        ...
        cost = calculate_costs(
            entry=entry, stop=stop, target=tp2,
...
def preoutcome_summary(candles: Iterable[Candle]) -> dict:
    """Safe structural audit only: bar/segment counts, never signals or returns."""
    series = validate_1h_candles(candles, require_regular_spacing=False)
    segments, gaps = split_1h_segments(series)
    return {
        "lab_id": LAB_ID,
        "timeframe": RESEARCH_TIMEFRAME,
        "closed_1h_candle_count": len(series),
        "contiguous_segment_count": len(segments),
        "detected_1h_gap_count": gaps,
