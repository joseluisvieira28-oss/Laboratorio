...
def load_manifest(source_root: Path) -> dict:
    p = source_root / "TFG_PBR01_4H_BINANCE_XVENUE_SOURCE_MANIFEST_V0.1.json"
    if not p.is_file():
...
def load_symbol(source_root: Path, manifest: dict, symbol: str) -> list[Candle]:
    start, end = ms(DISCOVERY_START), ms(DISCOVERY_END)
    rows: list[Candle] = []
...
def classify(base: dict, stress: dict, bootstrap: dict) -> tuple[str, list[str]]:
    ...
    return ("XVENUE_REPLICATED" if not failures else "XVENUE_NOT_REPLICATED"), failures
...
def run(source_root: Path, output: Path) -> int:
    ...
    params.validate()
    # FIRST BINANCE CROSS-VENUE OUTCOME INSPECTION POINT.
    ...
    ledger_path = output / "TFG_PBR01_4H_BINANCE_XVENUE_LEDGER_V0.1.json"
    ...
    closeout = {
        ...
        "temporal_oos_unlocked_automatically": False,
        "interpretation": "Cross-venue robustness evidence only; it cannot replace independent temporal OOS.",
    }
    (output / "TFG_PBR01_4H_BINANCE_XVENUE_CLOSEOUT_V0.1.json").write_text(json.dumps(closeout, indent=2, sort_keys=True), encoding="utf-8")
    ...
    return 0 if classification == "XVENUE_REPLICATED" else (5 if classification == "INSUFFICIENT_SAMPLE" else 3)
