...
import argparse,csv,datetime as dt,hashlib,json,math,os,re,urllib.request,zipfile
from pathlib import Path

BASE="https://data.binance.vision/data/futures/um/monthly/klines"
UA="CED1D-2025-SOURCE-GATE/0.1"
...
def audit_csv(zf,member,e):
    ...
    rows=0; seen=set(); prev=None; violations={k:0 for k in ["column_count","parse","timestamp_outside_month","non_ascending","duplicate_open_time","ohlc","negative_volume","negative_quote_volume","negative_trades","negative_taker_buy_base","negative_taker_buy_quote"]}
    first=last=None
    with zf.open(member) as raw:
        ...
        for line in text:
            ...
            try:
                ot=ms(parts[0]); o,h,l,c=map(float,parts[1:5]); vol=float(parts[5]); qv=float(parts[7]); trades=int(float(parts[8])); tb=float(parts[9]); tq=float(parts[10])
            except Exception:
                violations["parse"]+=1; continue
            rows+=1
            first=ot if first is None else first; last=ot
...
def main():
    ...
    a=ap.parse_args(); man=json.loads(Path(a.manifest).read_text()); out=Path(a.output); cache=Path(a.cache); out.mkdir(parents=True,exist_ok=True); cache.mkdir(parents=True,exist_ok=True)
    if man["expected_file_count"]!=28 or man["access_2026_plus"] or man["outcomes_authorized"]: raise SystemExit("MANIFEST_FIREWALL_FAIL")
    receipts=[]; failures=[]
    for i,e in enumerate(man["entries"],1):
        fn=e["filename"]; symbol=e["symbol"]; url=f"{BASE}/{symbol}/1m/{fn}"; zpath=cache/fn; cpath=cache/(fn+".CHECKSUM")
        fetch(url,zpath); fetch(url+".CHECKSUM",cpath)
        actual=sha256_file(zpath); checktext=cpath.read_text(errors="replace")
        checksum_hash=(re.search(r"([0-9a-fA-F]{64})",checktext) or [None,None])[1]
        ...
        try:
            ...
            with zipfile.ZipFile(zpath) as zf:
                ...
                if rec["member_pass"]: rec["csv_audit"]=audit_csv(zf,e["csv_member"],e)
                else: rec["csv_audit"]=None
        ...
        receipts.append(rec)
        print(f"[{i:02d}/28] {symbol} {e['month']} {'PASS' if ok else 'FAIL'}",flush=True)
    result={"document_id":"CED_1D_2025_SOURCE_GATE_RECEIPT_V0.1","status":"SOURCE_DATA_PASS" if not failures else "SOURCE_DATA_FAIL","manifest_document_id":man["document_id"],"file_count":len(receipts),"pass_count":sum(r["pass"] for r in receipts),"failures":failures,"confirmation_2025_outcomes_computed":False,"access_2026_plus":False,"receipts":receipts}
    raw=json.dumps(result,sort_keys=True,separators=(",",":")).encode(); result["fingerprint"]=hashlib.sha256(raw).hexdigest()
    (out/"CED_1D_2025_SOURCE_GATE_RECEIPT_V0.1.json").write_text(json.dumps(result,indent=2,sort_keys=True)+"\n")
