#!/usr/bin/env python3
"""CED1D-0031 prospective shadow collector V0.1.
...
UA="CED1D-0031-PROSPECTIVE-SHADOW/0.1"
...
def build_price_bars(start_day,end_day):
    bars=[]; source=[]
    d=start_day
    while d<=end_day:
        ...
        source.append({"kind":"klines","day":d.isoformat(),"url":url,"sha256":h,"rows":len(rows),"valid_day":bar["valid_day"]})
...
def main():
    ...
    start=FIRST_SIGNAL_DAY-timedelta(days=LOOKBACK)
    end=through+timedelta(days=HORIZON+1)
    source=[]
    try:
        bars,src=build_price_bars(start,end);source+=src
    ...
    receipt={"document_id":"CED1D_0031_PROSPECTIVE_SHADOW_RECEIPT_V0.1","status":status,
             ...
             "governance":{"pre_boundary_performance_backfill":False,"warmup_input_only":True,
