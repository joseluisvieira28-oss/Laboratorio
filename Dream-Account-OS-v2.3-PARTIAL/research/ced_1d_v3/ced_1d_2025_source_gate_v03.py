...
UA="CED1D-2025-SOURCE-GATE/0.3"
HEADER_FIRST_ALIASES={"open_time","opentime","timestamp","time","start_time"}
...
def audit_csv(zf,member,e):
    ...
    rows=0; seen=set(); prev=None; header_rows=0; first_nonempty=True
    violations={k:0 for k in ["column_count","parse","timestamp_outside_month","non_ascending","duplicate_open_time","ohlc","negative_volume","negative_quote_volume","negative_trades","negative_taker_buy_base","negative_taker_buy_quote","unexpected_header"]}
    first=last=None
    with zf.open(member) as raw:
        for bline in raw:
            ...
            if first_nonempty:
                ...
                if len(parts)==12 and norm_header(parts[0]) in HEADER_FIRST_ALIASES:
                    header_rows=1
                    continue
            if len(parts)!=12:
    ...
    return {"rows":rows,"unique_open_times":len(seen),"header_rows":header_rows,"first_open_time":first,"last_open_time":last,"violations":violations,"row_count_pass":rows==expected and len(seen)==expected,"header_rule_pass":header_rows in (0,1)}
...
def main():
    ...
    for i,e in enumerate(man["entries"],1):
        ...
        try:
            ...
            ok=rec["sha_match"] and rec["checksum_match"] and rec["zip_crc_pass"] and rec["member_pass"] and rec["csv_audit"]["row_count_pass"] and rec["csv_audit"]["header_rule_pass"] and not any(rec["csv_audit"]["violations"].values())
