...
P=ROOT/"ced1d_0031_prospective_shadow_collector_v02.py"
...
# Boundary constants.
assert m.FIRST_SIGNAL_DAY==date(2026,9,18)
assert m.FIRST_SIGNAL_COMPLETION.isoformat()=="2026-09-19T00:00:00+00:00"
assert m.FIRST_ENTRY_TS==int(datetime(2026,9,19,0,1,tzinfo=timezone.utc).timestamp()*1000)

# Frozen public-source URL shapes.
assert m.daily_kline_url("klines",date(2026,9,18)).endswith("/klines/AVAXUSDT/1m/AVAXUSDT-1m-2026-09-18.zip")
...
entry=datetime(2026,9,19,0,1,tzinfo=timezone.utc)
exitd=datetime(2026,9,20,0,1,tzinfo=timezone.utc)
ft=int(datetime(2026,9,19,8,0,tzinfo=timezone.utc).timestamp()*1000)
minute=(ft//60000)*60000
ev=[{"event_id":"CED1D-0031:2026-09-18","signal_day":"2026-09-18","status":"TRADE","direction":1,
     "entry_day":"2026-09-19","exit_day":"2026-09-20","entry_ts_ms":int(entry.timestamp()*1000),
...
assert m.complete_signal_weeks(date(2026,9,20))==[]
...
rows=[]
for i in range(10):
    sd=date(2026,9,18)+timedelta(days=i)
...
met=m.metrics_and_routing(rows,date(2026,10,1))
...
print("CED1D_0031_PROSPECTIVE_SHADOW_SYNTHETIC_QA_PASS")
...
bars,src=m.build_shadow_price_bars(date(2026,9,18))
prior=[b for b in bars if b["date"]<"2026-09-18" and b["valid_day"]]
assert len(prior)==20,(len(prior),calls)
assert len(calls)==21,(len(calls),calls)
assert calls[0]==date(2026,9,17) and calls[-1]==date(2026,8,28),(calls[0],calls[-1])
assert all(x["role"]=="LOOKBACK_INPUT_ONLY" for x in src),src
m.build_price_bars=_orig_build

print("CED1D_0031_PROSPECTIVE_SHADOW_V02_VALID_DAY_WARMUP_QA_PASS")
