#!/usr/bin/env python3
"""Aggregate the mandatory 20 PMD-001 chain-exact V0.6 cross-date rows."""
...
ARTIFACT_RE=re.compile(r'PMD-001-chain-exact-v06-row-(\d+)$')
...
def main()->int:
    ...
    stage_ok=all(r.get('stage')=='CHAIN_EXACT_SOURCE_REBUILD_V06' for r in rows)
    ...
    reconciliation_ok=(
        ...
        and malformed==0 and outcomes_closed and stage_ok
    )
    if not reconciliation_ok:
        verdict='CHAIN_EXACT_CROSSDATE_V06_TECHNICAL_INCOMPLETE'
    elif complete==EXPECTED_ROWS and eligible==EXPECTED_ROWS and unique_boundaries==EXPECTED_ROWS:
        verdict='CHAIN_EXACT_CROSSDATE_V06_PASS'
    else:
        verdict='CHAIN_EXACT_CROSSDATE_V06_FAIL'
    ...
    (out/'chain_exact_crossdate_v06_rows.jsonl').write_text(canon,encoding='utf-8')
    receipt={
        'lab':'PMD-001','stage':'CHAIN_EXACT_CROSSDATE_V06','economic_outcomes_opened':False,
    ...
    (out/'chain_exact_crossdate_v06_receipt.json').write_text(json.dumps(receipt,indent=2,sort_keys=True)+'\n',encoding='utf-8')
    print(json.dumps(receipt,indent=2,sort_keys=True))
    return 0 if verdict=='CHAIN_EXACT_CROSSDATE_V06_PASS' else 2
