...
def collect_one(rpc: Rpc, row: dict[str, Any], root: Path, store_full_blocks: bool) -> dict[str, Any]:
    mint = row["mint"]
    t0 = parse_ts(row["t0"])
...
def main() -> int:
    ...
    ap.add_argument("--out-dir", default="pmd_block_first_v02")
    ...
    if args.rpc_url:
        url = args.rpc_url
        source = args.source_name or "generic_rpc_probe_safe_cutoff"
    else:
        ...
        source = "helius_archival_rpc_block_first_safe_cutoff"
    ...
    with summary.open("w", encoding="utf-8") as sf:
        for row in rows:
            try:
                res = collect_one(rpc, row, root, args.store_full_blocks)
            except Exception as exc:
                res = {
                    "lab": "PMD-001",
                    "stage": "BLOCK_FIRST_REBUILD_V02_SAFE_CUTOFF",
