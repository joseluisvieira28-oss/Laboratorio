#!/usr/bin/env python3
"""PMD-001 frozen block-first source collector V0.1.

Outcome-blind. Signature history identifies the [T0-300s,T0) population; every
unique in-window slot is then fetched with full getBlock and all transaction
bodies come exclusively from those block responses.
"""
...
def collect_one(rpc: Rpc,row:dict[str,Any],root:Path)->dict[str,Any]:
    mint=row['mint']; t0=parse_ts(row['t0']); lo=t0-WINDOW_SECONDS
    ...
    inwin.sort(key=lambda x:(x.get('slot') or -1,x.get('signature') or ''))
    slots=sorted({int(x['slot']) for x in inwin if x.get('slot') is not None})
    ...
    for slot in slots:
        ...
        blocks[slot]=block
        evidence={
            'lab':'PMD-001','stage':'BLOCK_FIRST_REBUILD_V01','outcomes_opened':False,
    ...
    with raw_path.open('w',encoding='utf-8') as f:
        for meta in inwin:
            sig=meta['signature']; slot=int(meta['slot'])
            matches=find_matches(blocks.get(slot),sig)
            if len(matches)==0:
                missing+=1; tx=None
            elif len(matches)>1:
                duplicate_matches+=1; tx=matches[0]
            else:
                tx=matches[0]
            ...
            rec={
                ...
                'bonding_curve_pda':pda,'signature_meta':meta,
                'feature_eligible_by_time':True,'target_pump_mint_pda_present':is_target,
                'transaction':tx,'raw_response_sha256':sha256_json(tx),
            }
            f.write(json.dumps(rec,sort_keys=True,ensure_ascii=False)+'\n')

    complete=(
        conflicts==0 and not null_time and (crossed or exhausted) and
        block_errors==0 and missing==0 and duplicate_matches==0 and
        len(slots)>0 and len(inwin)>0
    )
    return {
        'lab':'PMD-001','stage':'BLOCK_FIRST_REBUILD_V01','outcomes_opened':False,
        'source_name':rpc.source_name,'mint':mint,'t0':row['t0'],'bonding_curve_pda':pda,
        'window_seconds':WINDOW_SECONDS,'signature_pages':pages,
        'signatures_unique_seen':len(by_sig),'in_window_signatures':len(inwin),
        'unique_in_window_slots':len(slots),'blocks_requested':len(slots),
        'block_errors':block_errors,'missing_signature_bodies':missing,
        'duplicate_signature_body_matches':duplicate_matches,'signature_conflicts':conflicts,
        'null_block_time_seen':null_time,'crossed_lower_bound':crossed,
        'history_exhausted':exhausted,'valid_target_pump_transactions':target,
        'source_complete':complete,'raw_file':str(raw_path),
...
def main()->int:
    ...
    ap.add_argument('--out-dir',default='pmd_block_first_v01')
    ...
    if args.rpc_url:
        url=args.rpc_url; source=args.source_name or 'generic_rpc_probe'
    else:
        key=os.environ.get('HELIUS_API_KEY')
        if not key:
            print('HELIUS_API_KEY required for authenticated archival route',file=sys.stderr); return 3
        url=f'https://mainnet.helius-rpc.com/?api-key={key}'; source='helius_archival_rpc_block_first'
    ...
    with summary.open('w',encoding='utf-8') as sf:
        for row in rows:
            try: res=collect_one(rpc,row,root)
                ...
                res={'lab':'PMD-001','stage':'BLOCK_FIRST_REBUILD_V01','outcomes_opened':False,
            ...
            print(json.dumps({k:res.get(k) for k in ('mint','source_complete','in_window_signatures','unique_in_window_slots','valid_target_pump_transactions','collector_error') if k in res},sort_keys=True))
    print(json.dumps({'lab':'PMD-001','stage':'BLOCK_FIRST_REBUILD_V01','outcomes_opened':False,
