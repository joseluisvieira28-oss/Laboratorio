...
OUT = OUTDIR / 'EDS_SOURCE_DATA_GATE_V02.json'
...
UA = {'User-Agent':'Mozilla/5.0 EXCHANGE-DELISTING-SHOCK-001/2.0','clienttype':'web'}
...
def find_articles(obj):
    ...
    uniq={}
    for a in out:
        uniq[str(a.get('code') or a.get('articleCode'))]=a
    ...
    def walk(x):
        ...
        if isinstance(x,dict):
            if isinstance(x.get('body'),str) and (x.get('title') or x.get('releaseDate')):
                if best is None or len(x.get('body',''))>len(best.get('body','')):
                    best=x
            for v in x.values():
                walk(v)
        elif isinstance(x,list):
...
def token_items(body):
    items=[]
    # Original HTML-list route.
    for m in re.finditer(r'(?is)<li[^>]*>(.*?)</li>', body):
        t=textify(m.group(1)).strip()
        mm=re.search(r'^(.*?)\s*\(([A-Z0-9]{2,15})\)\s*$', t)
        if mm:
            items.append((mm.group(2),mm.group(1).strip(' -•')))
    # Source-remediation route: Binance articles may render token bullets without
    # literal <li> elements in the CMS body. Parse newline-preserved plain text,
    # but later accept only symbols that also occur in the exact-pairs section.
    txt=textify(body)
    for m in re.finditer(r'(?m)^\s*[•\-*]?\s*([^\n()]{1,100}?)\s*\(([A-Z0-9]{2,15})\)\s*$', txt):
...
def delist_ts(text):
    # Semantics unchanged: full-token delist only. Remediation accepts Binance's
    # 2024 wording "all spot trading pairs" as equivalent to 2023 "all trading pairs".
    m=re.search(r'(?is)cease trading on all(?:\s+spot)?\s+trading pairs.{0,500}?\bat\s*(20\d\d-\d\d-\d\d)\s+(\d\d:\d\d)\s*\(UTC\)',text)
    if not m:
        return None
    return datetime.strptime(m.group(1)+' '+m.group(2),'%Y-%m-%d %H:%M').replace(tzinfo=timezone.utc)

def evidence_text(text):
    m=re.search(r'(?is)(.{0,140}cease trading on all(?:\s+spot)?\s+trading pairs.{0,500}?\(UTC\))',text)
...
def main():
    ...
    for code,a in sorted(candidates.items()):
        try:
            ...
            d=find_detail(j)
            if not d:
                raise RuntimeError('detail body absent')
            ...
            evtext=evidence_text(txt)
            for sym in bases:
                if sym not in names:
                    rejected.append({'article_code':code,'symbol':sym,'title':title,'reason':'TOKEN_NAME_NOT_EXACTLY_RESOLVED'})
                    continue
                qualified.append({
                    'article_code':code,
    ...
    if source_pass:
        for e in qualified:
            ...
            if not e['has_usdt_pair']:
                route.append({'symbol':e['token_symbol'],'article_code':e['article_code'],'qualified':False,'reason':'NO_USDT_PAIR_IN_OFFICIAL_DELIST_NOTICE'})
                continue
            if entry.year>=2025 or stop.year>=2025:
                ...
                continue
            a=checksum(e['token_symbol'],entry.date())
            b=checksum(e['token_symbol'],stop.date())
            route.append({
    ...
    if list_error and not candidates:
        classification='SOURCE_ACCESS_BLOCKED'
    elif not source_pass:
        classification='INSUFFICIENT_SOURCE_SAMPLE'
    elif not route_pass:
        classification='INSUFFICIENT_MARKET_ROUTE_SAMPLE'
    else:
        classification='SOURCE_DATA_PASS'

    rejected_counts=dict(Counter(x.get('reason','UNKNOWN') for x in rejected))
    doc={
      'lab_id':'EXCHANGE-DELISTING-SHOCK-001',
      'mve_id':'EDS-BINANCE-FULLTOKEN-001',
      'source_remediation_version':'V0.2',
      'mode':'SOURCE_DATA_GATE_ONLY',
      'receipt':{
        'classification':classification,
        'list_pages_requested':list_pages,
        'list_error':list_error,
        'candidate_title_count':len(candidates),
        'detail_failure_count':len(detail_failures),
        'rejected_reason_counts':rejected_counts,
        'qualified_token_events':len(qualified),
        'qualified_distinct_tokens':len(tokens),
        'qualified_years':years,
        'route_qualified_token_events':len(rq),
        'route_qualified_distinct_tokens':len(rq_tokens),
        'route_qualified_years':rq_years,
    ...
    if classification=='SOURCE_ACCESS_BLOCKED':
        raise SystemExit(21)
    if classification.startswith('INSUFFICIENT_'):
        raise SystemExit(22)
