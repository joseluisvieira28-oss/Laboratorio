...
def run(output: Path) -> int:
    ...
    if len(month_list) != EXPECTED_MONTHS_PER_SYMBOL:
        raise RuntimeError("technical failure: frozen month count mismatch")
    ...
    required = len(SYMBOLS) * EXPECTED_MONTHS_PER_SYMBOL
    ...
    status = (
        "SOURCE_AUDIT_PASS"
        if not errors and len(entries) == required and all(n == EXPECTED_MONTHS_PER_SYMBOL for n in by_symbol.values())
        else "SOURCE_OR_DATA_BLOCKED"
    )
    manifest = {
        "lab_id": LAB_ID,
        "stage": "BINANCE_PREPERIOD_SOURCE_GATE",
        "status": status,
