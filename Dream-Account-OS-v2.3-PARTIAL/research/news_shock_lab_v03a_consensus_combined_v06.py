...
from research.news_shock_lab_v03a_consensus_combined_v05 import load_authority as load_parent_authority
...
FREEZE6_PATH = ROOT / "NEWS_SHOCK_LAB_V03A_CONSENSUS_SEED_TRANCHE06_FREEZE_V0.1.json"
SEED6_PATH = ROOT / "NEWS_SHOCK_LAB_V03A_CONSENSUS_SEED_TRANCHE06_V0.1.json"

EXPECTED_FREEZE6 = "d833dbdd508d1ddcbcd0ffadafc7dbd90a65c3a47514f6e93a9c73306f467ba1"
EXPECTED_SEED6 = "03899ec02f291b16021fad636d021d85b08ba5596cec50843896e0d818de2c13"
EXPECTED_PARENT_COMBINED = "8c111516b2841d53e927241584ac9af510a4ad294008d5bb83dee472a71846eb"
EXPECTED_PARENT_VALIDATION = "3c7d537831a271301f2b9de478ac7619aa4e97dd5ed9bc4826d8a493c08e92e4"
EXPECTED_PROVENANCE_FREEZE = "a78d4933663e687a61f91a548c074239b046c13e96dc059a73ba27828715ee7a"
EXPECTED_COMPLETE_A = 23
EXPECTED_INCOMPLETE = 21
...
def load_authority():
    ...
    freeze6 = _load_hashed(FREEZE6_PATH, EXPECTED_FREEZE6, "freeze6")
    seed6 = _load_hashed(SEED6_PATH, EXPECTED_SEED6, "seed6")

    if freeze6["parent_combined_manifest_fingerprint"] != EXPECTED_PARENT_COMBINED:
        raise PermissionError("freeze6 parent combined manifest mismatch")
    if freeze6["parent_validation_fingerprint"] != EXPECTED_PARENT_VALIDATION:
        raise PermissionError("freeze6 parent validation mismatch")
    if freeze6["parent_provenance_freeze_fingerprint"] != EXPECTED_PROVENANCE_FREEZE:
        raise PermissionError("freeze6 parent provenance freeze mismatch")
    if seed6["freeze_fingerprint"] != freeze6["fingerprint"]:
        raise PermissionError("seed6 freeze mismatch")
    ...
    for key in (
        ...
        if freeze6["authority"].get(key) is not False:
            raise PermissionError(f"freeze6 authority guard drift {key}")

    for key, value in seed6.get("guards", {}).items():
        if value is not False:
            raise PermissionError(f"seed6 guard drift: {key}")
    ...
    ids6 = set(seed6["events"])
    if prior_ids & ids6:
        raise PermissionError("tranche06 overlaps prior consensus seed tranches")
    if ids6 != set(freeze6["scope"]["event_ids"]):
        raise PermissionError("seed6 scope mismatch")

    _validate_seed(seed6, "seed6")
    return (
        seed1,
        freeze2,
        seed2,
...
def build_combined(output_dir: Path):
    ...
    combined_events = {}
    for seed in (seed1, seed2, seed3, seed4, seed5, seed6):
    ...
    manifest = {
        "document_type": "NEWS_SHOCK_LAB_V03A_CPI_CONSENSUS_COMBINED_MANIFEST_V06",
        "version": "0.3A-v06",
        "status": "CONSENSUS_TRANCHE06_ATTACHED_VALIDATION_REQUIRED",
    ...
    manifest["status"] = "CONSENSUS_VALIDATED_23_COMPLETE_A_21_INCOMPLETE"
    unsigned = deepcopy(manifest)
    unsigned.pop("fingerprint", None)
    manifest["fingerprint"] = canonical_hash(unsigned)

    (output_dir / "combined_v06_consensus_manifest.json").write_text(
        json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    (output_dir / "combined_v06_validation_receipt.json").write_text(
    ...
    (output_dir / "combined_v06_summary.json").write_text(
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) != 2:
        raise SystemExit("usage: news_shock_lab_v03a_consensus_combined_v06 <output_dir>")
