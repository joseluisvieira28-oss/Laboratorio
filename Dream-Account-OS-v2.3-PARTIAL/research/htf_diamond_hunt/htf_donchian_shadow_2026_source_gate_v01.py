...
END=1788220800000    # 2026-09-01 UTC
MIN=60_000; BAR15=900_000
...
UA='HTF-DONCHIAN-SHADOW-001 Phase6 2026 source gate/1.0'
...
OUT=ROOT/'research'/'local_data'/'htf_donchian_shadow_2026_source_v01'
PCAN=OUT/'canonical_15m'; FCAN=OUT/'canonical_funding'
RECEIPT=OUT/'HTF_DONCHIAN_SHADOW_001_2026_SOURCE_GATE_V0.1.json'
...
def main():
    auth=json.loads(AUTH.read_text()); freeze=json.loads(FREEZE.read_text())
    assert auth['status']=='EXPLICIT_2026_SHADOW_DATA_ACCESS_AUTHORIZED'
    assert auth['scope']['mode']=='PAPER_SHADOW_ONLY' and auth['scope']['year']==2026
    assert freeze['status']=='FROZEN_BEFORE_2026_MARKET_ACCESS'
    assert freeze['dataset']['months']==list(MONTHS)
    assert freeze['dataset']['2025_access_allowed'] is False
    assert freeze['dataset']['september_2026_partial_access_allowed'] is False
    OUT.mkdir(parents=True,exist_ok=True); PCAN.mkdir(exist_ok=True); FCAN.mkdir(exist_ok=True)
    price_records=[]; fund_records=[]; audits={}
    try:
        for s in SYMS:
            rows=[]; fund=[]
            for ym in MONTHS:
                pn=f'{s}-1m-{ym}.zip'; pu=f'{PBASE}/{s}/1m/{pn}'
                pb=get(pu); ps=csum(get(pu+'.CHECKSUM'),pn); pl=sha(pb)
                if ps!=pl: raise ValueError(f'price checksum mismatch:{s}:{ym}')
                rr=list(price_rows(pb,pn.replace('.zip','.csv'))); rows.extend(rr)
                price_records.append({'symbol':s,'month':ym,'archive_name':pn,'archive_url':pu,'local_sha256':pl,'provider_sha256':ps,'row_count':len(rr)})
                fn=f'{s}-fundingRate-{ym}.zip'; fu=f'{FBASE}/{s}/{fn}'
                fb=get(fu); fs=csum(get(fu+'.CHECKSUM'),fn); fl=sha(fb)
                if fs!=fl: raise ValueError(f'funding checksum mismatch:{s}:{ym}')
                fr=funding_rows(fb,fn.replace('.zip','.csv')); fund.extend(fr)
                fund_records.append({'symbol':s,'month':ym,'archive_name':fn,'archive_url':fu,'local_sha256':fl,'provider_sha256':fs,'event_count':len(fr)})
            ...
            audits[s]={'raw_minute_count':len(rows),'complete_15m_count':complete,'incomplete_15m_count':inc,'detected_minute_gap_count':gaps,'canonical_15m_sha256':sha(pp.read_bytes()),'funding_event_count':len(fund),'canonical_funding_sha256':sha(fp.read_bytes())}
        pre={'experiment_id':'HTF-DONCHIAN-SHADOW-001','phase':'PHASE_6_2026_PROTECTED_SHADOW_BLOCK_SOURCE_GATE','provider':'Binance public data','market':'USD-M perpetual futures','months':list(MONTHS),'price_records':sorted(price_records,key=lambda x:(x['symbol'],x['month'])),'funding_records':sorted(fund_records,key=lambda x:(x['symbol'],x['month'])),'audits':audits}
        fp=chash(pre)
        out={'status':'SOURCE_DATA_PASS',**pre,'source_fingerprint':fp,'signal_calculation_performed':False,'return_calculation_performed':False,'outcome_evaluation_performed':False,'access_2025_performed':False,'access_2026_performed':True,'access_partial_september_2026_performed':False,'live_trading':False,'exchange_mutation':False,'orders_created':False}
        ...
        out={'status':'BLOCKED_PRE_OUTCOME_SOURCE_GATE','reason':f'{type(e).__name__}: {e}','outcome_evaluation_performed':False,'access_2025_performed':False,'access_2026_performed':True,'access_partial_september_2026_performed':False,'live_trading':False,'exchange_mutation':False,'orders_created':False}
        RECEIPT.parent.mkdir(parents=True,exist_ok=True); RECEIPT.write_text(json.dumps(out,indent=2)); print(json.dumps(out,indent=2),file=sys.stderr); return 2
