...
SETUP_ID = "PBR05_TIMEFRAME_TRANSPORT_BREAKOUT_RETEST_LONG_1D"
...
class ResearchParameters1D:
    ...
    def validate(self) -> None:
        if self.timeframe != RESEARCH_TIMEFRAME:
            raise ValueError("TFG PBR01 transport is frozen to 1d only")
        integer_fields = {
...
def validate_1d_candles(candles: Iterable[Candle], *, require_regular_spacing: bool = True) -> list[Candle]:
    ...
    for candle in series:
        ...
        if candle.open_time % ONE_DAY_MS != 0:
            raise ValueError("1d candle is not UTC-day aligned")
        ...
        if previous_open is not None:
            ...
            if require_regular_spacing and candle.open_time - previous_open != ONE_DAY_MS:
                raise ValueError("1d candle series contains a gap")
...
def aggregate_15m_to_1d(candles: Iterable[Candle]) -> list[Candle]:
    ...
    for candle in closed:
        ...
        bucket = candle.open_time - (candle.open_time % ONE_DAY_MS)
...
def derive_signal_geometries_1d(candles: Iterable[Candle], parameters: ResearchParameters1D, costs: CostAssumptions) -> list[SignalGeometry]:
    parameters.validate()
    costs.validate()
    ...
    for breakout_index in range(minimum_history, len(series) - 2):
        ...
        payload = {
            ...
            "full_trade_blockers": ("research_only_no_live_authority",),
        }
        output.append(SignalGeometry(**payload, fingerprint=_canonical_hash(payload)))
    return output


def simulate_outcome_1d(signal: SignalGeometry, candles: Iterable[Candle], costs: CostAssumptions, *, max_holding_bars: int) -> TradeOutcome:
    costs.validate()
    series = validate_1d_candles(candles, require_regular_spacing=True)
    ...
    def finish(reason: str, candle: Candle, price: float, bars_held: int, tp1_reached: bool, ambiguity: bool) -> TradeOutcome:
        ...
        return TradeOutcome(
            ...
            exit_reason=reason,
            exit_open_time=candle.open_time,
            exit_price=price,
            bars_held=bars_held,
...
def evaluate_symbol_1d(symbol: str, candles: Iterable[Candle], parameters: ResearchParameters1D, costs: CostAssumptions):
    segments, gap_count = split_1d_segments(candles)
    records: list[ResearchTradeRecord1D] = []
    raw_geometry_count = net_rr_rejected_count = overlap_skipped_count = 0
    for segment in segments:
        signals = derive_signal_geometries_1d(segment, parameters, costs)
        ...
        for signal in (s for s in signals if s.geometry_status == "GEOMETRY_READY"):
            if active_until is not None and signal.entry_open_time <= active_until:
                ...
                continue
            outcome = simulate_outcome_1d(signal, segment, costs, max_holding_bars=parameters.max_holding_bars)
            records.append(ResearchTradeRecord1D(symbol, signal, outcome))
...
def evaluate_universe_1d(candles_by_symbol: Mapping[str, Iterable[Candle]], parameters: ResearchParameters1D, costs: CostAssumptions):
    all_records: list[ResearchTradeRecord1D] = []
    totals = Counter()
    ...
    for symbol in sorted(candles_by_symbol):
        ...
        records, diagnostics = evaluate_symbol_1d(symbol, raw, parameters, costs)
    ...
    if data_open_times and selected_count:
        span_days = (max(data_open_times) - min(data_open_times) + ONE_DAY_MS) / 86_400_000
        if span_days > 0:
    ...
    metrics = EvaluationMetrics(
        ...
        unresolved_trade_count=selected_count - resolved_count,
        ...
        stop_gap_rate=_safe_rate(sum(o.exit_reason == "STOP_GAP" for o in outcomes), resolved_count),
...
def reprice_fixed_cohort_1d(records: Iterable[ResearchTradeRecord1D], costs: CostAssumptions, *, min_net_rr: float = 2.0) -> FixedCohortCostMetrics:
    costs.validate()
    rows = list(records)
    values: list[float] = []
    violations = 0
    ...
    return FixedCohortCostMetrics(
        cost_scenario=costs.name,
        cohort_source="BASE_SENSITIVITY_SELECTED_TFG_PBR01_1D_TRADES",
        ...
        median_net_r=median(values) if values else None,
        profit_factor_r=_profit_factor(values),
        net_rr_below_minimum_count=violations,
        net_rr_violation_rate=_safe_rate(violations, len(rows)),
    )


def classify_discovery_1d(base: EvaluationMetrics, stress: FixedCohortCostMetrics, bootstrap: BootstrapInterval) -> DiscoveryDecision1D:
    minimum = 100
    ...
    return DiscoveryDecision1D(**payload, fingerprint=_canonical_hash(payload))


def records_as_dict_1d(records: Iterable[ResearchTradeRecord1D]) -> list[dict]:
