...
from research.news_shock_lab_v03a_consensus_combined_v07 import load_authority as load_parent_authority
...
FREEZE8_PATH = ROOT / "NEWS_SHOCK_LAB_V03A_CONSENSUS_SEED_TRANCHE08_FREEZE_V0.1.json"
SEED8_PATH = ROOT / "NEWS_SHOCK_LAB_V03A_CONSENSUS_SEED_TRANCHE08_V0.1.json"

EXPECTED_FREEZE8 = "43744f6953f734eab08733032f70964008f93bb3457ae01ba76c5958440dc9d2"
EXPECTED_SEED8 = "681bde65cf0c76b2b3de92820a8deced319fd8873d5905dd2e89e60945467ef8"
EXPECTED_PARENT_COMBINED = "aaa6d864fb3ba71a9a407e47a1c3830b368469f5ff6c00df1380de7a5ca1d5d7"
EXPECTED_PARENT_VALIDATION = "a92e746a076a268d7473b449fe8fb5e4120ed734013368544c9b4855cda80246"
EXPECTED_PROVENANCE_FREEZE = "a78d4933663e687a61f91a548c074239b046c13e96dc059a73ba27828715ee7a"
EXPECTED_COMPLETE_A = 33
EXPECTED_INCOMPLETE = 11
...
def load_authority():
    ...
    freeze8 = _load_hashed(FREEZE8_PATH, EXPECTED_FREEZE8, "freeze8")
    seed8 = _load_hashed(SEED8_PATH, EXPECTED_SEED8, "seed8")

    if freeze8["parent_combined_manifest_fingerprint"] != EXPECTED_PARENT_COMBINED:
        raise PermissionError("freeze8 parent combined manifest mismatch")
    if freeze8["parent_validation_fingerprint"] != EXPECTED_PARENT_VALIDATION:
        raise PermissionError("freeze8 parent validation mismatch")
    if freeze8["parent_provenance_freeze_fingerprint"] != EXPECTED_PROVENANCE_FREEZE:
        raise PermissionError("freeze8 parent provenance freeze mismatch")
    if seed8["freeze_fingerprint"] != freeze8["fingerprint"]:
        raise PermissionError("seed8 freeze mismatch")
    ...
    for key in (
        ...
        if freeze8["authority"].get(key) is not False:
            raise PermissionError(f"freeze8 authority guard drift {key}")
    for key, value in seed8.get("guards", {}).items():
        if value is not False:
            raise PermissionError(f"seed8 guard drift: {key}")
    ...
    ids8 = set(seed8["events"])
    if prior_ids & ids8:
        raise PermissionError("tranche08 overlaps prior consensus seed tranches")
    if ids8 != set(freeze8["scope"]["event_ids"]):
        raise PermissionError("seed8 scope mismatch")
    _validate_seed(seed8, "seed8")
    return (
        seed1, freeze2, seed2, freeze3, seed3, amend4, freeze4, seed4,
...
def build_combined(output_dir: Path):
    ...
    combined_events = {}
    for seed in (seed1, seed2, seed3, seed4, seed5, seed6, seed7, seed8):
    ...
    manifest = {
        "document_type": "NEWS_SHOCK_LAB_V03A_CPI_CONSENSUS_COMBINED_MANIFEST_V08",
        "version": "0.3A-v08",
        "status": "CONSENSUS_TRANCHE08_ATTACHED_VALIDATION_REQUIRED",
    ...
    manifest["status"] = "CONSENSUS_VALIDATED_33_COMPLETE_A_11_INCOMPLETE"
    unsigned = deepcopy(manifest)
    unsigned.pop("fingerprint", None)
    manifest["fingerprint"] = canonical_hash(unsigned)
    (output_dir / "combined_v08_consensus_manifest.json").write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    (output_dir / "combined_v08_validation_receipt.json").write_text(json.dumps(validation, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    summary = {
    ...
    (output_dir / "combined_v08_summary.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) != 2:
        raise SystemExit("usage: news_shock_lab_v03a_consensus_combined_v08 <output_dir>")
