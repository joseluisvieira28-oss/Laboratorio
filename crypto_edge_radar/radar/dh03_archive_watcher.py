...
from .binance_usdm_archive import (
    BinanceArchiveError,
    SYMBOLS,
    load_verified_daily_1m_rows,
)
from .binance_usdm_monthly_funding import (
    BinanceMonthlyFundingError,
    load_verified_monthly_funding,
)
from .dh03_12h_forward import (
    FIRST_SIGNAL_BAR_OPEN_MS,
    MAX_HOLD_MS,
    MinuteBar,
    aggregate_12h,
    derive_signals,
...
def _completed_months_through(latest_day:date)->list[str]:
    """Months ending strictly before latest_day's UTC month."""
...
def _load_price_corpus(latest_day:date,timeout:int=30):
    ...
    for s in SYMBOLS:
        for d in _days(WARMUP_START,latest_day):
            ...
            source.append({
                "symbol":s,"day":d.isoformat(),"sha256":row["sha256"],
                "row_count":len(row["rows"]),"checksum_verified":row["checksum_verified"],
                "forward_role":"FORWARD_OR_EXECUTION" if d>=FIRST_ARCHIVE_FORWARD_DAY else "WARMUP_ONLY",
            })
...
def _load_final_funding(latest_day:date,timeout:int=30):
    ...
    for ym in _completed_months_through(latest_day):
        ...
        for s,row in month_rows.items():
            ...
            receipts.append({
                "symbol":s,"month":ym,"sha256":row["sha256"],
                "event_count":row["event_count"],"checksum_verified":True,
            })
...
def evaluate_corpus(
    minutes_by_symbol:dict[str,list[MinuteBar]],
    funding_by_symbol:dict[str,tuple[list[int],list[float]]],
    funding_complete_through_ms:int,
)->dict[str,Any]:
    symbols={}
    totals={
        "signals":0,"price_exits":0,"final_resolutions":0,
        "funding_pending":0,"unresolved_price_paths":0,"overlap_skipped":0,
    }
    for s in SYMBOLS:
        ...
        for sig in signals:
            ...
            active_until=(
                provisional.exit_time
                if provisional.exit_time is not None
                else sig.entry_open_time+MAX_HOLD_MS
            )
            entry={
                "signal":asdict(sig),
                "price_exit_time":provisional.exit_time,
                "price_exit_price":provisional.exit_price,
                "price_exit_reason":provisional.exit_reason,
                "price_path_unresolved":provisional.execution_path_unresolved,
                "funding_reconciliation_pending":True,
        ...
        symbols[s]={
            "complete_12h_bars":len(bars),
            "incomplete_12h_buckets":incomplete,
...
def _persist(store,receipt:dict[str,Any])->dict[str,int]:
    ...
    for s,row in receipt["evaluation"]["symbols"].items():
        for item in row["rows"]:
            ...
            fp=sig["fingerprint"]
            signal_payload={
                "strategy_id":STRATEGY_ID,"symbol":s,
                "signal":sig,"source_mode":"BINANCE_PUBLIC_ARCHIVE_CHECKSUM_VERIFIED",
                "used_as_forward_evidence":True,
                "authenticated_exchange_api_used":False,
                "orders_created":False,"exchange_mutation_performed":False,
            }
            r=store.append_once("DH03_FORWARD_SIGNAL",f"{STRATEGY_ID}:signal:{fp}",signal_payload)
            inserted["signals"]+=int(bool(r["inserted"]))
            if item["price_exit_time"] is not None and not item["price_path_unresolved"]:
                price_payload={
                    "strategy_id":STRATEGY_ID,"symbol":s,"signal_fingerprint":fp,
                    "exit_time":item["price_exit_time"],"exit_price":item["price_exit_price"],
                    "exit_reason":item["price_exit_reason"],
                    "funding_reconciliation_pending":item["funding_reconciliation_pending"],
                    "authenticated_exchange_api_used":False,
                    "orders_created":False,"exchange_mutation_performed":False,
                }
                r=store.append_once("DH03_FORWARD_PRICE_EXIT",f"{STRATEGY_ID}:price-exit:{fp}",price_payload)
                inserted["price_exits"]+=int(bool(r["inserted"]))
            if item["final_resolution"] is not None:
                final_payload={
                    "strategy_id":STRATEGY_ID,"symbol":s,"signal_fingerprint":fp,
                    "resolution":item["final_resolution"],
                    "funding_authority":"BINANCE_PUBLIC_DATA_MONTHLY_FUNDINGRATE_CHECKSUM_VERIFIED",
                    "authenticated_exchange_api_used":False,
                    "orders_created":False,"exchange_mutation_performed":False,
                }
                r=store.append_once("DH03_FORWARD_FINAL_RESOLUTION",f"{STRATEGY_ID}:final:{fp}",final_payload)
                inserted["final_resolutions"]+=int(bool(r["inserted"]))
    snap_key=f"{STRATEGY_ID}:snapshot:{receipt['latest_archive_day']}"
    snap={
        "strategy_id":STRATEGY_ID,
        "latest_archive_day":receipt["latest_archive_day"],
        "status":receipt["status"],
        "totals":receipt["evaluation"]["totals"],
        "funding_complete_through_ms":receipt["funding_complete_through_ms"],
        "source_price_file_count":len(receipt["price_source_receipts"]),
        "source_funding_file_count":len(receipt["funding_source_receipts"]),
        "authenticated_exchange_api_used":False,
        "orders_created":False,"exchange_mutation_performed":False,
        "live_capital_enabled":False,
    }
    r=store.append_once("DH03_FORWARD_SNAPSHOT",snap_key,snap)
    inserted["snapshots"]+=int(bool(r["inserted"]))
    return inserted
...
def run_once(*,now:datetime|None=None,persist:bool=True,store=None)->dict[str,Any]:
    ...
    base={
        ...
        "latest_archive_day":latest_day.isoformat(),
    ...
    if latest_day<FIRST_ARCHIVE_FORWARD_DAY:
        return {
            **base,
            ...
            "first_forward_archive_day":FIRST_ARCHIVE_FORWARD_DAY.isoformat(),
    ...
    receipt={
        **base,
        "status":"OK",
        "first_signal_bar_open_ms":FIRST_SIGNAL_BAR_OPEN_MS,
        "warmup_start":WARMUP_START.isoformat(),
        "price_source_receipts":price_receipts,
        "funding_source_receipts":funding_receipts,
        "funding_missing":missing,
        "funding_complete_through_ms":complete_through,
...
def main()->int:
    try:
        ...
        print(json.dumps({
            "strategy_id":STRATEGY_ID,"status":"FAIL_CLOSED",
