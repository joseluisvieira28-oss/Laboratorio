...
REQUIRED_TRANSLATION = "NEGATIVE_SIGNAL_TO_MEXC_USDT_PERPETUAL_SHORT_V0.1"
CURRENT_PLACE_ORDER_PATH = "/api/v1/private/order/create"

class Tier2LiveGateError(RuntimeError):
    pass

def _load(path: str) -> dict[str, Any]:
    payload=json.loads(Path(path).read_text(encoding="utf-8"))
    if not isinstance(payload,dict):
        raise Tier2LiveGateError(f"JSON object required: {path}")
    return payload
...
def _parse_utc(value: Any, field: str)->datetime:
    try:
        ...
        raise Tier2LiveGateError(f"invalid {field}") from exc
    if dt.tzinfo is None:
        raise Tier2LiveGateError(f"{field} must be timezone-aware")
...
def _positive(value: Any, field: str)->float:
    ...
    if not isfinite(out) or out<=0:
        raise Tier2LiveGateError(f"invalid {field}")
    return out

def validate_tier2_options_short_execution(
    *,
    authority_path: str,
    preflight_path: str,
    signal_path: str,
    ...
    if authority.get("policy_id")!="TIER2-MICROLIVE-POLICY-V1.0-FROZEN-2026-09-24":
        blockers.append("TIER2_POLICY_BINDING_MISSING")
    ...
    if authority.get("direction")!=REQUIRED_DIRECTION:
        blockers.append("AUTHORITY_DIRECTION_NOT_SHORT")
    if authority.get("execution_translation")!=REQUIRED_TRANSLATION:
        blockers.append("EXECUTION_TRANSLATION_NOT_FROZEN")
    ...
    max_notional=float(authority.get("max_notional_usdt",-1))
    daily_stop=float(authority.get("daily_realized_loss_kill_usdt",-1))
    weekly_stop=float(authority.get("rolling_7d_realized_loss_kill_usdt",-1))
    if abs(max_notional-10.0)>1e-12:
        blockers.append("MAX_NOTIONAL_NOT_FROZEN_10_USDT")
    if abs(daily_stop-2.0)>1e-12:
        blockers.append("DAILY_KILL_NOT_FROZEN_2_USDT")
    if abs(weekly_stop-5.0)>1e-12:
        blockers.append("WEEKLY_KILL_NOT_FROZEN_5_USDT")
    ...
    try:
        p_at=_parse_utc(preflight.get("checked_at_utc"),"preflight.checked_at_utc")
        age=(now-p_at).total_seconds()
        if age>max_preflight_age:
            blockers.append("AUTHENTICATED_PREFLIGHT_STALE")
        if age<0:
            blockers.append("AUTHENTICATED_PREFLIGHT_FROM_FUTURE")
    except Tier2LiveGateError:
    ...
    try:
        r_at=_parse_utc(risk.get("as_of_utc"),"risk_state.as_of_utc")
        age=(now-r_at).total_seconds()
        if age>max_risk_age:
    ...
    daily_loss=float(risk.get("daily_realized_loss_usdt",999))
    weekly_loss=float(risk.get("rolling_7d_realized_loss_usdt",999))
    if daily_loss>=daily_stop:
        blockers.append("DAILY_HALT_ACTIVE")
    if weekly_loss>=weekly_stop:
        blockers.append("WEEKLY_HALT_ACTIVE")
    if int(risk.get("open_micro_live_positions",999))!=0:
    ...
    try:
        ...
        contract_size=_positive(contract.get("contract_size_base"),"contract_size")
        reference_price=_positive(contract.get("reference_price"),"reference_price")
        min_vol=_positive(contract.get("min_contract_volume"),"min_contract_volume")
        vol_step=_positive(contract.get("contract_volume_step"),"contract_volume_step")
        volume=int(authority.get("volume_contracts"))
        if volume<min_vol:
            blockers.append("ORDER_VOLUME_BELOW_VENUE_MINIMUM")
        step_units=(volume-min_vol)/vol_step
        if abs(step_units-round(step_units))>1e-9:
            blockers.append("ORDER_VOLUME_NOT_ON_VENUE_STEP")
        requested_notional=volume*contract_size*reference_price
        venue_min=_positive(contract.get("minimum_executable_notional_estimate_usdt"),"minimum_executable_notional_estimate_usdt")
        if venue_min>max_notional+1e-12:
            blockers.append("VENUE_MINIMUM_EXCEEDS_10_USDT_CAP")
        if requested_notional>max_notional+1e-12:
            blockers.append("REQUESTED_NOTIONAL_EXCEEDS_10_USDT_CAP")
    except Exception:
        blockers.append("ORDER_SIZE_OR_NOTIONAL_CAP_INVALID")

    projected_rt=None
    effective_taker=None
    try:
        budget=float(authority.get("friction_budget_bps"))
        projected_rt=float(authority.get("projected_round_trip_bps"))
        effective_taker=float((checks.get("fees") or {}).get("effective_taker_fee_bps_for_execution_model"))
        if budget!=20.0:
            blockers.append("OPTIONS_STRESS20_FRICTION_BUDGET_NOT_FROZEN")
        if projected_rt<2.0*effective_taker:
            blockers.append("PROJECTED_FRICTION_BELOW_FEE_FLOOR")
        if projected_rt>budget:
    ...
    try:
        ...
        if abs((exit_target-target).total_seconds()-24*3600)>1e-9:
            blockers.append("EXIT_TARGET_NOT_EXACTLY_PLUS_24H")
        max_late=float(authority.get("max_late_seconds"))
        seconds_from_target=(now-target).total_seconds()
        if seconds_from_target<-60:
            blockers.append("ENTRY_WINDOW_NOT_ARMED")
        elif seconds_from_target>max_late:
            blockers.append("STALE_SIGNAL_NO_CHASE")
    except Exception:
        blockers.append("ENTRY_OR_EXIT_WINDOW_INVALID")

    try:
        reference_entry=_positive(authority.get("reference_entry_price"),"reference_entry_price")
    except Tier2LiveGateError:
    ...
    return {
        "gate_id":"MEXC_OPTIONS_V21_SHORT_TIER2_MICROLIVE_GATE_V0.3",
        ...
        "daily_realized_loss_kill_usdt":daily_stop,
        "rolling_7d_realized_loss_kill_usdt":weekly_stop,
        ...
        "execution_translation":REQUIRED_TRANSLATION,
...
__all__=["EXECUTION_TOKEN","Tier2LiveGateError","validate_tier2_options_short_execution"]
