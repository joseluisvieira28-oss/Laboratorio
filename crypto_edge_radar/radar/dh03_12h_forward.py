...
BASE_COST=0.002
STRESS_COST=0.003
FIRST_SIGNAL_BAR_OPEN_MS=1789732800000  # 2026-09-18T12:00:00Z
...
def aggregate_12h(minutes:list[MinuteBar])->tuple[list[Bar12H],int]:
    ...
    for b in sorted(buckets):
        ...
        if len(rows)!=720 or any(rows[i].open_time!=b+i*MIN_MS for i in range(len(rows))):
            ...
            continue
        out.append(Bar12H(
            b,rows[0].open,max(x.high for x in rows),min(x.low for x in rows),rows[-1].close
        ))
...
def derive_signals(symbol:str,bars:list[Bar12H])->tuple[list[Signal],dict[str,int]]:
    ...
    for i in range(max(LOOKBACK,ATR_LEN),len(bars)-1):
        ...
        payload={
            "symbol":symbol,"signal_open_time":s.open_time,
            "entry_open_time":bars[i+1].open_time,"entry":entry,
            "stop":stop,"target":target,"risk":risk
        }
        signals.append(Signal(
            symbol,s.open_time,bars[i+1].open_time,entry,stop,target,risk,canonical_hash(payload)
        ))
...
def simulate_signal(
    ...
    if i>=len(minutes) or mtimes[i]!=sig.entry_open_time:
        return Trade(sig.symbol,sig.signal_open_time,sig.entry_open_time,None,sig.entry,None,sig.stop,sig.target,sig.initial_risk_fraction,
                     "EXECUTION_PATH_UNRESOLVED_ENTRY",None,None,0,None,None,None,True)
    ...
    prev_t=None
    exit_t=None; px=None; reason=None
    for j in range(i,len(minutes)):
        ...
        if prev_t is not None and b.open_time!=prev_t+MIN_MS:
            return Trade(sig.symbol,sig.signal_open_time,sig.entry_open_time,None,sig.entry,None,sig.stop,sig.target,sig.initial_risk_fraction,
                         "EXECUTION_PATH_UNRESOLVED_GAP",None,None,0,None,None,None,True)
        prev_t=b.open_time
        if b.open_time==deadline:
    ...
    if exit_t is None or px is None or reason is None:
        return Trade(sig.symbol,sig.signal_open_time,sig.entry_open_time,None,sig.entry,None,sig.stop,sig.target,sig.initial_risk_fraction,
                     "UNRESOLVED_END_OF_AVAILABLE_SOURCE",None,None,0,None,None,None,True)
    ...
    return Trade(sig.symbol,sig.signal_open_time,sig.entry_open_time,exit_t,sig.entry,px,sig.stop,sig.target,sig.initial_risk_fraction,
                 reason,price_gross,funding,nfund,econ,base,stress,False)
...
def evaluate_symbol(
    ...
    return {
        ...
        "resolved_trades":sum(not x.execution_path_unresolved for x in rows),
        "unresolved_trades":sum(x.execution_path_unresolved for x in rows),
        "trades":[asdict(x) for x in rows],
    }
