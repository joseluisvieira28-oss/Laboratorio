#!/usr/bin/env python3
"""CED1D-0031 prospective Render shadow collector V0.3.
...
FIRST_SIGNAL_DAY=date(2026,9,22)
FIRST_SIGNAL_COMPLETION=datetime(2026,9,23,0,0,tzinfo=timezone.utc)
FIRST_ENTRY_TS=int(datetime(2026,9,23,0,1,tzinfo=timezone.utc).timestamp()*1000)

LOOKBACK=20
HORIZON=1
WARMUP_MAX_CALENDAR_DAYS=120
...
PUBLIC_API="https://fapi.binance.com"
UA="CED1D-0031-RENDER-SHADOW/0.3"
...
def public_funding(start_ms,end_ms):
    ...
    for r in rows:
        ...
        if "markPrice" not in r:raise GateError(f"FUNDING_MARKPRICE_MISSING:{r}")
        ft=int(r["fundingTime"]); rate=float(r["fundingRate"]); mark_price=float(r["markPrice"])
        ...
        if not math.isfinite(mark_price) or mark_price<=0:raise GateError(f"FUNDING_MARKPRICE_NUMERIC:{r}")
...
def main():
    ...
    if activation["activation_scope"]["first_eligible_signal_completion"]!="2026-09-23T00:00:00Z":raise GateError("BOUNDARY_MISMATCH")
