...
from radar.dh03_12h_forward import (
    ATR_LEN,BAR_MS,FIRST_SIGNAL_BAR_OPEN_MS,LOOKBACK,MinuteBar,Signal,
    aggregate_12h,atr_series,derive_signals,simulate_signal
)
...
class DH0312HForwardTests(unittest.TestCase):
    ...
    def test_atr_is_wilder_after_seed(self):
        ...
        self.assertIsNotNone(atr[ATR_LEN+1])
    ...
    def test_same_minute_stop_target_is_stop_first(self):
        ...
        minutes=[
            MinuteBar(entry_t,100.0,104.0,98.0,101.0),
            MinuteBar(entry_t+60_000,101.0,101.2,100.5,101.0),
        ]
        row=simulate_signal(sig,minutes,[],[])
        self.assertEqual(row.exit_reason,"STOP_WINS_SAME_MINUTE_AMBIGUITY")
        self.assertEqual(row.exit_price,99.0)
        self.assertFalse(row.execution_path_unresolved)
    ...
    def test_long_funding_is_negative_sum_between_entry_and_exit(self):
        ...
        minutes=[
            MinuteBar(entry_t,100.0,100.2,99.8,100.1),
            MinuteBar(entry_t+60_000,100.1,101.1,100.0,101.0),
        ]
        row=simulate_signal(sig,minutes,[entry_t,entry_t+30_000],[0.001,0.002])
        self.assertEqual(row.exit_reason,"TARGET")
        self.assertAlmostEqual(row.funding_return,-0.002,12)
        self.assertEqual(row.funding_event_count,1)

    def test_gap_fails_closed(self):
        entry_t=FIRST_SIGNAL_BAR_OPEN_MS+BAR_MS
        sig=Signal("BTCUSDT",FIRST_SIGNAL_BAR_OPEN_MS,entry_t,100.0,90.0,130.0,0.1,"x")
        minutes=[
            MinuteBar(entry_t,100.0,101.0,99.0,100.0),
            MinuteBar(entry_t+120_000,100.0,101.0,99.0,100.0),
        ]
        row=simulate_signal(sig,minutes,[],[])
        self.assertTrue(row.execution_path_unresolved)
        self.assertEqual(row.exit_reason,"EXECUTION_PATH_UNRESOLVED_GAP")
