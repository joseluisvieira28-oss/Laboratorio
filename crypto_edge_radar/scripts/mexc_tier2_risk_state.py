...
def build_state(*,preflight_path:Path,receipt_root:Path,now:datetime|None=None)->dict[str,Any]:
    ...
    open_micro=max(open_positions,len(active))
    blockers=[]
    ...
    if daily_loss>=2.0:
        blockers.append("DAILY_2_USDT_KILL_ACTIVE")
    if rolling_loss>=5.0:
        blockers.append("ROLLING_7D_5_USDT_KILL_ACTIVE")
    if open_micro>1:
        blockers.append("MULTIPLE_MICRO_LIVE_POSITIONS_PRESENT")

    return {
        "risk_state_id":"MEXC_TIER2_ACCOUNT_RISK_STATE_V0.3",
        ...
        "open_micro_live_positions":open_micro,
...
def main()->int:
    ...
    ap.add_argument("--out",default="mexc_tier2_account_risk_state.json")
    args=ap.parse_args()
    state=build_state(preflight_path=Path(args.preflight),receipt_root=Path(args.receipt_root))
